...
class MaterializedViewIntegrationTester:
    ...
    def test_materialized_view_performance(self):
        ...
        for cache_key, description in test_cases:
            ...
            if result:
                ...
                print(
                    f"   📊 Materialized View: {avg_mv_time:.2f}ms avg ({record_count} records)"
                )
                ...
                print(
                    f"   📊 Direct Query: {avg_direct_time:.2f}ms avg ({direct_count} records)"
                )
    ...
    def test_fallback_behavior(self):
        ...
        set_result = self.cache.set(
            cache_key, test_data["custom_dropdown_data"], timeout=300
        )
        if set_result:
    ...
    def test_batch_operations(self):
        ...
        for key, data in results.items():
            if data:
                if (
                    "people_dropdown" in key
                    or "location_dropdown" in key
                    or "asset_dropdown" in key
                ):
                    mv_hits += 1
    ...
    def test_cache_statistics(self):
        ...
        if hasattr(self.cache, "get_stats"):
            ...
            print(
                f"   • Average data size: {stats.get('avg_data_size_bytes', 0)} bytes"
            )
    ...
    def benchmark_comprehensive_performance(self):
        ...
        scenarios = [
            ...
            (
                "Batch mixed lookup",
                lambda: self.cache.get_many(
                    [
                        "people_dropdown_batch",
                        "location_dropdown_batch",
                        "custom_1",
                        "custom_2",
                    ]
                ),
...
def main():
    ...
    # Determine exit code
    success = all(
        result for result in test_results.values() if isinstance(result, bool)
    )
