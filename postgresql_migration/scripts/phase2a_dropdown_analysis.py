...
sys.path.append(
    "/home/satyam/Documents/YOUTILITY-MIGRATION-DJANGO5-POSTGRESQL/YOUTILITY3"
)
...
class DropdownAnalyzer:
    ...
    def _analyze_model_for_dropdown_potential(self, model):
        ...
        try:
            ...
            dropdown_indicators = {
                "has_name_field": any(
                    f in fields for f in ["name", "title", "description"]
                ),
                "has_enable_field": "enable" in fields,
                "has_tenant_field": "tenant" in fields or "tenant_id" in fields,
                "has_foreign_keys": len(
                    [f for f in model._meta.fields if f.is_relation]
                )
                > 0,
            ...
            try:
                ...
                dropdown_indicators["small_dataset"] = (
                    count < 10000
                )  # Arbitrary threshold
            ...
            if score >= 5:  # Threshold for consideration
                ...
                if dropdown_indicators["small_dataset"]:
                    reasons.append(
                        f"manageable size ({dropdown_indicators['record_count']} records)"
                    )
        ...
        try:
            with connection.cursor() as cursor:
                ...
                if has_pg_stat:
                    ...
                    print(
                        "⚠️  pg_stat_statements not available, using alternative analysis"
                    )
    ...
    def identify_materialized_view_candidates(self):
        ...
        for candidate in candidates:
            ...
            if any(
                word in model.__name__.lower() for word in ["people", "user", "person"]
            ):
                priority_score += 5
            ...
            if any(word in model.__name__.lower() for word in ["location", "site"]):
                priority_score += 4
            if any(
                word in model.__name__.lower()
                for word in ["type", "category", "priority"]
            ):
                priority_score += 3
        ...
        for i, candidate in enumerate(priority_candidates[:10], 1):
            print(
                f"   {i}. {candidate['app']}.{candidate['name']} "
    ...
    def estimate_performance_benefits(self):
        ...
        for candidate in candidates[:5]:  # Top 5 candidates
            ...
            try:
                ...
                print(
                    f"   Expected improvement: {(query_time/estimated_mv_time):.1f}x faster"
                )
    ...
    def run_complete_analysis(self):
        ...
        print("📋 Summary:")
        print(
            f"   • {len(self.analysis_results.get('dropdown_candidates', []))} dropdown candidates identified"
        )
        print(
            f"   • {len(recommendations['immediate_candidates'])} immediate implementation targets"
        )
