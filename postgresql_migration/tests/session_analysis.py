...
class SessionAnalyzer:
    ...
    def test_current_configuration(self):
        ...
        print(
            f"   SESSION_CACHE_ALIAS: {getattr(settings, 'SESSION_CACHE_ALIAS', 'Not set')}"
        )
        print(
            f"   SESSION_COOKIE_AGE: {getattr(settings, 'SESSION_COOKIE_AGE', 'Not set')} seconds"
        )
        print(
            f"   SESSION_SAVE_EVERY_REQUEST: {getattr(settings, 'SESSION_SAVE_EVERY_REQUEST', 'Not set')}"
        )
        ...
        if session_cache_alias:
            try:
                ...
                if result == "test_value":
                    ...
                    print(
                        f"   ❌ Session cache '{session_cache_alias}' failed to retrieve test data"
                    )
    ...
    def measure_session_performance(self, num_tests=100):
        ...
        print(
            f"   Creation:  {creation_time:.3f}s total, {creation_time/num_tests*1000:.2f}ms avg"
        )
        print(
            f"   Retrieval: {retrieval_time:.3f}s total, {retrieval_time/num_tests*1000:.2f}ms avg"
        )
        print(
            f"   Deletion:  {deletion_time:.3f}s total, {deletion_time/num_tests*1000:.2f}ms avg"
        )
    ...
    def explain_postgresql_optimization(self):
        ...
        print(
            "   Before: SESSION_ENGINE = 'django.contrib.sessions.backends.cached_db'"
        )
