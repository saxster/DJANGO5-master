...
def show_session_configuration():
    ...
    print(
        f"🔄 Save Every Request: {getattr(settings, 'SESSION_SAVE_EVERY_REQUEST', False)}"
    )
    print(
        f"🚪 Expire at Browser Close: {getattr(settings, 'SESSION_EXPIRE_AT_BROWSER_CLOSE', False)}"
    )
...
"""
    ...
    print(
        "python manage.py shell                      # Interactive session inspection"
    )
