...
def get_webcaps_choices():  # sourcery skip: merge-list-append
    ...
    parent_menus = Capability.objects.get_web_caps_for_client_orm()
    for i in parent_menus:
        logger.info(f"depth: {i.depth} capscode {i.capscode} path {i.path}")
    choices, temp = [], []
...
def create_bv_reportting_heirarchy(instance, newcode, newtype, parent):
    if instance.id is None:
        ...
        # update bu tree
        if (
            instance.bucode not in (None, "NONE")
            and hasattr(instance.parent, "bucode")
            and instance.parent.bucode in (None, "NONE")
        ):
...
def check_if_image_format_is_correct(image_format):
    if image_format not in ["image/png", "image/jpeg", "image/jpg"]:
        return [
            False,
            "Image Extension is not correct. Correct Extension should be (.png,.jpeg,.jpg)",
        ]
    return [True, " "]
...
def is_bulk_image_data_correct(data):
    ...
    for image_data in data:
        ...
        is_image_format_correct = check_if_image_format_is_correct(
            image_format=image_format
        )
        is_peoplecode_of_image_exists = (
            check_if_the_people_code_with_the_image_name_exists(image_name=image_name)
        )
        errors_message = []
...
class BulkImageUploader:
    ...
    def download_single_image(
        self, image_data: Dict, destination_path: str
    ) -> Tuple[bool, str]:
...
def save_image_and_image_path(
    drive_link: str, media_root: str
) -> Tuple[int, List[Dict]]:
    """
    ...
    try:
        ...
        with concurrent.futures.ThreadPoolExecutor(
            max_workers=uploader.max_workers
        ) as executor:
            future_to_image = {
                executor.submit(
