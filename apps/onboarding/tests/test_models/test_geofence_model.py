...
class TestGeofenceMasterModel:
    ...
    def test_geofence_str_representation(self, geofence_factory):
        ...
        geofence = geofence_factory(gfcode="TEST001", gfname="Test Geofence")
    ...
    def test_geofence_polygon_field(self, geofence_factory, gps_coordinates):
        ...
        geofence = geofence_factory(geofence=gps_coordinates["office_polygon"])
    ...
    def test_geofence_relationships(
        self, geofence_factory, test_client_bt, test_bu_bt, test_people_onboarding
    ):
    ...
    def test_geofence_unique_constraint(self, geofence_factory, test_bu_bt):
        ...
        geofence1 = geofence_factory(gfcode="UNIQUE001", bu=test_bu_bt)

        # Try to create duplicate - should raise IntegrityError
        with pytest.raises(IntegrityError):
            geofence_factory(gfcode="UNIQUE001", bu=test_bu_bt)
    ...
    def test_geofence_alert_text_field(self, geofence_factory):
        ...
        for alert_text in alert_messages:
            geofence = geofence_factory(
                alerttext=alert_text, gfcode=f"ALERT_{alert_text[:5].upper()}"
            )
    ...
    def test_geofence_enable_disable_functionality(self, geofence_factory):
        ...
        enabled_geofence = geofence_factory(
            gfcode="ENABLED001", gfname="Enabled Geofence", enable=True
        )

        disabled_geofence = geofence_factory(
            gfcode="DISABLED001", gfname="Disabled Geofence", enable=False
        )
    ...
    def test_geofence_bulk_operations(
        self, geofence_factory, test_client_bt, test_bu_bt
    ):
        ...
        bulk_enabled = GeofenceMaster.objects.filter(
            gfcode__startswith="BULK_", enable=True
        )
        bulk_disabled = GeofenceMaster.objects.filter(
            gfcode__startswith="BULK_", enable=False
        )
        ...
        # Test bulk update
        GeofenceMaster.objects.filter(gfcode__startswith="BULK_").update(
            alerttext="Updated bulk alert text"
        )

        # Verify bulk update
        updated_geofences = GeofenceMaster.objects.filter(
            gfcode__startswith="BULK_", alerttext="Updated bulk alert text"
        )
        assert updated_geofences.count() == 20

    def test_geofence_filtering_by_client_and_bu(
        self, geofence_factory, test_client_bt, test_bu_bt
    ):
    ...
    def test_geofence_point_containment(self, geofence_factory, gps_coordinates):
        ...
        assert office_geofence.geofence.touches(
            boundary_point
        ) or office_geofence.geofence.contains(boundary_point)
    ...
    def test_geofence_performance_queries(
        self, geofence_factory, test_client_bt, test_bu_bt
    ):
        ...
        # Test count queries
        total_count = GeofenceMaster.objects.filter(gfcode__startswith="PERF_").count()
        assert total_count == 100

        # Test enabled/disabled filtering
        enabled_count = GeofenceMaster.objects.filter(
            gfcode__startswith="PERF_", enable=True
        ).count()
        disabled_count = GeofenceMaster.objects.filter(
            gfcode__startswith="PERF_", enable=False
        ).count()
        ...
        client_geofences = GeofenceMaster.objects.filter(
            gfcode__startswith="PERF_", client=test_client_bt
        )
        bu_geofences = GeofenceMaster.objects.filter(
            gfcode__startswith="PERF_", bu=test_bu_bt
        )

        assert client_geofences.count() == 100
        assert bu_geofences.count() == 100

    def test_geofence_alert_configuration(
        self, geofence_factory, test_people_onboarding
    ):
