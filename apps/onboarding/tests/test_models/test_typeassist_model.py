...
class TestTypeAssistModel:
    ...
    def test_typeassist_str_representation(self, typeassist_factory):
        ...
        ta = typeassist_factory(tacode="TESTCODE", taname="Test Type Assist")
    ...
    def test_typeassist_hierarchy_parent_child(self, typeassist_factory):
        ...
        parent_ta = typeassist_factory(tacode="PARENT_TYPE", taname="Parent Type")

        child_ta = typeassist_factory(
            tacode="CHILD_TYPE", taname="Child Type", tatype=parent_ta
        )
    ...
    def test_typeassist_unique_constraint(self, typeassist_factory, test_client_bt):
        ...
        ta1 = typeassist_factory(
            tacode="UNIQUE", tatype=parent_ta, client=test_client_bt
        )

        # Try to create duplicate - should raise IntegrityError
        with pytest.raises(IntegrityError):
            typeassist_factory(tacode="UNIQUE", tatype=parent_ta, client=test_client_bt)

    def test_typeassist_relationships(
        self, typeassist_factory, test_client_bt, test_bu_bt
    ):
    ...
    def test_typeassist_get_all_children(self, typeassist_factory):
        ...
        root_ta = typeassist_factory(tacode="ROOT", taname="Root Type")

        level1_ta = typeassist_factory(
            tacode="LEVEL1", taname="Level 1 Type", tatype=root_ta
        )

        level2_ta = typeassist_factory(
            tacode="LEVEL2", taname="Level 2 Type", tatype=level1_ta
        )
    ...
    def test_typeassist_circular_reference_validation(self, typeassist_factory):
        ...
        ta1 = typeassist_factory(tacode="TYPE1", taname="Type 1")

        ta2 = typeassist_factory(tacode="TYPE2", taname="Type 2", tatype=ta1)
    ...
    def test_typeassist_common_types(self, typeassist_factory):
        ...
        client_type = typeassist_factory(tacode="CLIENT", taname="Client")

        site_type = typeassist_factory(tacode="SITE", taname="Site")

        # Test designation types
        security_guard = typeassist_factory(
            tacode="SECURITY_GUARD", taname="Security Guard"
        )

        supervisor = typeassist_factory(tacode="SUPERVISOR", taname="Supervisor")

        # Test event types
        checkin_type = typeassist_factory(tacode="CHECKIN", taname="Check In")

        checkout_type = typeassist_factory(tacode="CHECKOUT", taname="Check Out")
    ...
    def test_typeassist_enable_disable_functionality(self, typeassist_factory):
        ...
        ta_enabled = typeassist_factory(
            tacode="ENABLED", taname="Enabled Type", enable=True
        )

        ta_disabled = typeassist_factory(
            tacode="DISABLED", taname="Disabled Type", enable=False
        )
    ...
    def test_typeassist_tenant_aware_functionality(
        self, typeassist_factory, test_client_bt
    ):
        """Test TypeAssist tenant-aware model functionality"""
        ta = typeassist_factory(
            tacode="TENANT_TEST", taname="Tenant Test Type", client=test_client_bt
        )
    ...
    def test_typeassist_bulk_operations(self, typeassist_factory):
        ...
        bulk_enabled = TypeAssist.objects.filter(
            tacode__startswith="BULK_", enable=True
        )
        bulk_disabled = TypeAssist.objects.filter(
            tacode__startswith="BULK_", enable=False
        )
        ...
        # Test bulk update
        TypeAssist.objects.filter(tacode__startswith="BULK_").update(
            taname="Updated Bulk Type"
        )

        # Verify bulk update
        updated_types = TypeAssist.objects.filter(
            tacode__startswith="BULK_", taname="Updated Bulk Type"
        )
    ...
    def test_typeassist_hierarchy_categories(self, typeassist_factory):
        ...
        bv_identifier = typeassist_factory(
            tacode="BVIDENTIFIER", taname="Business View Identifier"
        )

        designation = typeassist_factory(tacode="DESIGNATION", taname="Designation")

        # Create subcategories under BV Identifier
        client_id = typeassist_factory(
            tacode="CLIENT", taname="Client", tatype=bv_identifier
        )

        site_id = typeassist_factory(tacode="SITE", taname="Site", tatype=bv_identifier)

        # Create subcategories under Designation
        guard = typeassist_factory(
            tacode="GUARD", taname="Security Guard", tatype=designation
        )

        supervisor = typeassist_factory(
            tacode="SUPERVISOR", taname="Supervisor", tatype=designation
        )
    ...
    def test_typeassist_deep_hierarchy(self, typeassist_factory):
        ...
        main_category = typeassist_factory(tacode="MAIN", taname="Main Category")

        # Level 1: Sub category
        sub_category = typeassist_factory(
            tacode="SUB", taname="Sub Category", tatype=main_category
        )

        # Level 2: Sub-sub category
        sub_sub_category = typeassist_factory(
            tacode="SUBSUB", taname="Sub-Sub Category", tatype=sub_category
        )

        # Level 3: Specific type
        specific_type = typeassist_factory(
            tacode="SPECIFIC", taname="Specific Type", tatype=sub_sub_category
        )
    ...
    def test_typeassist_filtering_by_client_and_bu(
        self, typeassist_factory, test_client_bt, test_bu_bt
    ):
        ...
        bu_specific = typeassist_factory(
            tacode="BU_SPECIFIC", taname="BU Specific Type", bu=test_bu_bt
        )
        ...
        global_type = typeassist_factory(tacode="GLOBAL", taname="Global Type")
    ...
    def test_typeassist_performance_queries(self, typeassist_factory):
        ...
        # Test count queries
        total_count = TypeAssist.objects.filter(tacode__startswith="PERF_").count()
        ...
        enabled_count = TypeAssist.objects.filter(
            tacode__startswith="PERF_", enable=True
        ).count()
        disabled_count = TypeAssist.objects.filter(
            tacode__startswith="PERF_", enable=False
        ).count()
    ...
    def test_typeassist_self_reference_prevention(self, typeassist_factory):
        ...
        ta = typeassist_factory(tacode="SELF_REF", taname="Self Reference Test")
