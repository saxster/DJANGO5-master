...
class TestBtModel:
    ...
    def test_bt_str_representation(self, bt_factory):
        ...
        bt = bt_factory(bucode="TESTCODE", buname="Test Business Unit")
    ...
    def test_bt_hierarchy_parent_child(self, bt_factory):
        ...
        parent_bt = bt_factory(bucode="PARENT", buname="Parent BU")

        child_bt = bt_factory(bucode="CHILD", buname="Child BU", parent=parent_bt)
    ...
    def test_bt_gps_location_field(self, bt_factory, gps_coordinates):
        ...
        bt = bt_factory(gpslocation=gps_coordinates["bangalore_office"], gpsenable=True)
    ...
    def test_bt_preferences_json_field(self, bt_factory, bu_preference_data):
        ...
        bt = bt_factory(bupreferences=bu_preference_data)
    ...
    def test_bt_unique_constraint(self, bt_factory, test_identifier_client):
        ...
        # Create first BU
        bt1 = bt_factory(
            bucode="UNIQUE", parent=parent_bt, identifier=test_identifier_client
        )
    ...
    def test_bt_site_includes_array_field(self, bt_factory):
        ...
        bt = bt_factory(siincludes=site_includes)
    ...
    def test_bt_solid_field(self, bt_factory):
        ...
        bt = bt_factory(solid="SOL12345")

        assert bt.solid == "SOL12345"

    def test_bt_relationships(
        self, bt_factory, test_identifier_client, test_bu_type, test_people_onboarding
    ):
    ...
    def test_bt_butree_field(self, bt_factory):
        ...
        bt = bt_factory(butree="Root/Client/Region/Site")
    ...
    def test_bt_bulk_operations(self, bt_factory):
        ...
        # Test bulk filtering
        enabled_bus = Bt.objects.filter(bucode__startswith="BULK_", enable=True)
        disabled_bus = Bt.objects.filter(bucode__startswith="BULK_", enable=False)

        assert enabled_bus.count() == 5
        assert disabled_bus.count() == 5

        # Test bulk update
        Bt.objects.filter(bucode__startswith="BULK_").update(gpsenable=True)

        # Verify bulk update
        updated_bus = Bt.objects.filter(bucode__startswith="BULK_", gpsenable=True)
        assert updated_bus.count() == 10

    def test_bt_filtering_queries(
        self, bt_factory, test_identifier_client, test_identifier_site
    ):
        """Test various filtering queries on Bt"""
        # Create BUs with different identifiers
        client_bt = bt_factory(bucode="CLIENT001", identifier=test_identifier_client)
        site_bt = bt_factory(bucode="SITE001", identifier=test_identifier_site)
    ...
    def test_bt_deep_hierarchy(self, bt_factory):
        ...
        root_bt = bt_factory(bucode="ROOT", buname="Root Organization")

        client_bt = bt_factory(
            bucode="CLIENT", buname="Client Organization", parent=root_bt
        )

        region_bt = bt_factory(
            bucode="REGION", buname="Regional Office", parent=client_bt
        )

        site_bt = bt_factory(bucode="SITE", buname="Site Location", parent=region_bt)
    ...
    def test_bt_gps_and_distance_combined(self, bt_factory, gps_coordinates):
        ...
        bt = bt_factory(
            gpslocation=gps_coordinates["bangalore_office"], pdist=200.0, gpsenable=True
        )
    ...
    def test_bt_sleeping_guard_and_audit_flags(self, bt_factory):
        ...
        bt = bt_factory(enablesleepingguard=True, skipsiteaudit=True)
    ...
    def test_bt_tenant_aware_functionality(self, bt_factory):
        ...
        bt = bt_factory(bucode="TENANT_TEST", buname="Tenant Test BU")
    ...
    def test_bt_performance_queries(self, bt_factory):
        ...
        # Test count queries
        total_count = Bt.objects.filter(bucode__startswith="PERF_").count()
        assert total_count == 100

        # Test enabled/disabled filtering
        enabled_count = Bt.objects.filter(
            bucode__startswith="PERF_", enable=True
        ).count()
        disabled_count = Bt.objects.filter(
            bucode__startswith="PERF_", enable=False
        ).count()
