    ...
    def clean(self):
        ...
        if not instance.pk and newcode and newtype and parent:  # Only for new records
            ...
            existing = obm.Bt.objects.filter(
                ...
                parent=parent, 
                identifier=newtype,
                cdtz__gte=recent_cutoff  # Recently created
            ).first()
            
            if existing:
                # This is likely a duplicate submission - return the existing record ID
...
class BuPrefForm(forms.Form):
    ...
    pvideolength = forms.IntegerField(initial=10, label="Panic Video Length (sec)", required=False)
...
class ClentForm(BuPrefForm):
    ...
    def clean(self):
        ...
        # Temporarily disabled - validation can be re-enabled when needed
        # if not cleaned_data.get("mobilecapability") and not cleaned_data.get(
        #     "webcapability"
        # ):
