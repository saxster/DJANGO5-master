...
class TestMqttClient(TestCase):
    ...
    def test_mqtt_client_initialization(self):
        ...
        self.assertEqual(
            self.mock_client_instance.on_connect, self.mqtt_client.on_connect
        )
        self.assertEqual(
            self.mock_client_instance.on_message, self.mqtt_client.on_message
        )
        self.assertEqual(
            self.mock_client_instance.on_disconnect, self.mqtt_client.on_disconnect
        )
    ...
    def test_on_message_status_topic(self, mock_log, mock_get_task_status):
        ...
        msg_mock.payload.decode.return_value = json.dumps(
            {"taskIds": [{"taskId": "task1"}, {"taskId": "task2"}]}
        )
        msg_mock.mid = 124
    ...
    def test_on_message_empty_payload(self, mock_log):
        ...
        mock_log.info.assert_any_call(
            "message: 125 from MQTT broker on topicunknown/topic payload not recieved"
        )
    ...
    def test_on_message_mutation_missing_fields(self, mock_log, mock_process_task):
        ...
        msg_mock = Mock()
        msg_mock.topic = "graphql/mutation"
        msg_mock.payload.decode.return_value = json.dumps({"mutation": "test mutation"})
        msg_mock.mid = 127
...
class TestMqttClientIntegration(TestCase):
    ...
    def test_full_mqtt_client_workflow(self, mock_mqtt_client):
        ...
        with patch.object(mqtt_client, "on_connect") as mock_on_connect, patch.object(
            mqtt_client, "on_message"
        ) as mock_on_message, patch.object(
