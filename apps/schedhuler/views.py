...
class Schd_I_TourFormJob(LoginRequiredMixin, View):
    ...
    def process_valid_schd_tourform(self, request, form, create):
        ...
        try:
            ...
            logger.critical(f"Unexpected error saving guard tour: {e}", exc_info=True)
            resp = rp.JsonResponse(
                {"error": "Failed to save guard tour due to unexpected error"}, status=500
            )
            raise e
        else:
...
class Schd_E_TourFormJob(LoginRequiredMixin, View):
    ...
    def process_valid_schd_tourform(request, form, create):
        ...
        try:
            ...
            logger.critical("external tour form is processing failed", exc_info=True)
            resp = rp.JsonResponse(
...
class InternalTourScheduling(LoginRequiredMixin, View):
    ...
    def post(self, request, *args, **kwargs):
        ...
        logger.info(f"Raw formData FULL LENGTH: {len(full_form_data)}")
        logger.info(f"Raw formData FIRST 200 chars: {full_form_data[:200]}...")
        logger.info(f"Raw formData LAST 200 chars: ...{full_form_data[-200:]}")
        logger.info(f"Raw formData contains gracetime: {'gracetime' in full_form_data}")
        logger.info(f"Raw formData contains cron: {'cron' in full_form_data}")
        logger.info(f"Raw formData contains fromdate: {'fromdate' in full_form_data}")
...
class ExternalTourScheduling(LoginRequiredMixin, View):
    ...
    def post(self, request, *args, **kwargs):
        ...
        logger.info(f"POST keys: {list(R.keys())}")
        logger.info(f"formData received: '{request.POST.get('formData', 'NOT_FOUND')}'")
        logger.info(f"Processed formData length: {len(str(formData))}")
        try:
            if R.get("postType") == "saveCheckpoint":
    ...
    def handle_valid_form(form, request, P):
        try:
            ...
            logger.critical(
                "external tour form, handle valid form failed", exc_info=True
            )
            return utils.handle_Exception(request)
