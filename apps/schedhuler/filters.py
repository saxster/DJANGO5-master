...
def assigned_to_qs(queryset, name, value):
    return queryset.filter(
        Q(aaatop__peoplename__icontains=value) | Q(pgroup__groupname__icontains=value)
    )
...
class SchdTourFilter(JobFilter):
    jobname = dfs.CharFilter(
        field_name="jobname", lookup_expr="icontains", label="Name"
    )
    assignedto = dfs.CharFilter(method=assigned_to_qs, label="People/Group")
    planduration = dfs.CharFilter(
        field_name="planduration", lookup_expr="icontains", label="Duration"
    )
    expirytime = dfs.CharFilter(
        field_name="expirytime", lookup_expr="icontains", label="Exp Time"
    )
    gracetime = dfs.CharFilter(
        field_name="gracetime", lookup_expr="icontains", label="Grace Time"
    )
    fromdate = dfs.DateTimeFilter(
        field_name="fromdate", lookup_expr="icontains", label="From"
    )
    uptodate = dfs.DateTimeFilter(
        field_name="uptodate", lookup_expr="icontains", label="To"
    )
...
class SchdTaskFilter(SchdTourFilter):
    asset = dfs.CharFilter(
        field_name="asset__assetname", lookup_expr="icontains", label="Asset"
    )
...
class InternalTourFilter(dfs.FilterSet):
    ...
    plandatetime = dfs.DateFromToRangeFilter(
        widget=wg.RangeWidget(attrs={"placeholder": "YYYY/MM/DD"})
    )
    jobdesc = dfs.CharFilter(
        field_name="jobdesc", lookup_expr="icontains", label="Description"
    )
    jobstatus = dfs.ChoiceFilter(
        field_name="jobstatus",
    ...
    performedby = dfs.CharFilter(
        field_name="performedby", lookup_expr="icontains", label="Performed By"
    )
    expirydatetime = dfs.DateTimeFilter(
        field_name="expirydatetime", label="Exp. Datetime"
    )
...
class TaskListJobneedFilter(InternalTourFilter):
    asset = dfs.CharFilter(
        field_name="asset__assetname", lookup_expr="icontains", label="Asset/Smartplace"
    )
    qset = dfs.CharFilter(
        field_name="qset__qset_name", lookup_expr="icontains", label="QuestionSetz"
    )
...
class TicketListFilter(JobneedFilter):
    ...
    assignedto = dfs.CharFilter(method=assigned_to_qs, label="People/Group")
    performedby = dfs.CharFilter(
        field_name="performedby__peoplename", lookup_expr="icontains"
    )
    ticketcategory = dfs.CharFilter(
        field_name="ticketcategory__taname", lookup_expr="icontains"
    )
    jobstatus = dfs.ChoiceFilter(
        field_name="jobstatus",
