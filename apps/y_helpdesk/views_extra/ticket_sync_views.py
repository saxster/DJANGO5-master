"""
Ticket Sync Views for Mobile REST API

Provides sync and delta pull endpoints for Ticket records.
...
class TicketSyncView(APIView):
    """POST /api/v1/helpdesk/sync/ - Bulk upsert tickets."""
    permission_classes = [IsAuthenticated]

    def post(self, request):
        """Process bulk ticket sync from mobile client."""
        try:
...
class TicketChangesView(APIView):
    """GET /api/v1/helpdesk/changes/ - Delta sync for tickets."""
    permission_classes = [IsAuthenticated]

    def get(self, request):
        """Get ticket changes for delta sync."""
        try:
            request_serializer = DeltaSyncRequestSerializer(data=request.query_params)
            request_serializer.is_valid(raise_exception=True)

            sync_service = TicketSyncService()
            result = sync_service.get_ticket_changes(
                user=request.user,
            ...
            serialized_items = TicketSyncSerializer(result['items'], many=True).data
