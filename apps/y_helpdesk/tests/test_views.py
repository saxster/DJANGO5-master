...
class EscalationMatrixViewTest(TestCase):
    def setUp(self):
        ...
        self.bt = Bt.objects.create(buname="Test BU", bucode="TESTBU001")
        ...
        self.group = Pgroup.objects.create(groupname="Test Group")
    ...
    def test_get_load_peoples_action(self, mock_get_peoples):
        mock_get_peoples.return_value = [{"id": 1, "peoplename": "Test Person"}]
    ...
    def test_get_load_groups_action(self, mock_get_groups):
        mock_get_groups.return_value = [{"id": 1, "groupname": "Test Group"}]
    ...
    def test_get_list_action(self, mock_get_listview):
        mock_get_listview.return_value = [{"id": 1, "level": 1, "frequency": "HOUR"}]
    ...
    def test_get_escalation_levels_action(self, mock_get_levels):
        mock_get_levels.return_value = [{"level": 1, "name": "Level 1"}]
    ...
    def test_get_reminder_config_action(self, mock_get_config):
        mock_get_config.return_value = [{"frequency": "HOUR", "frequencyvalue": 2}]
    ...
    @patch(
        "apps.y_helpdesk.views.EscalationMatrix.objects.handle_esclevel_form_postdata"
    )
    ...
    @patch(
        "apps.y_helpdesk.views.EscalationMatrix.objects.handle_reminder_config_postdata"
    )
...
class TicketViewTest(TestCase):
    ...
    def test_post_create_ticket(
        self, mock_store_history, mock_save_userinfo, mock_get_db
    ):
        ...
        with patch.object(self.view.params["form"], "is_valid", return_value=True):
            with patch.object(self.view.params["form"], "save") as mock_save:
                ...
                with patch.object(
                    self.view.params["form"], "__init__", return_value=None
                ):
                    response = self.view.post(request)
    ...
    def test_post_invalid_form(self):
        ...
        with patch(
            "apps.y_helpdesk.views.utils.get_current_db_name", return_value="default"
        ):
            with patch.object(self.view.params["form"], "is_valid", return_value=False):
                with patch.object(
                    self.view.params["form"],
                    "errors",
                    {"ticketdesc": ["This field is required"]},
                ):
                    with patch(
                        "apps.y_helpdesk.views.utils.handle_invalid_form"
                    ) as mock_handle_invalid:
                        mock_handle_invalid.return_value = JsonResponse(
                            {"errors": "Invalid form"}, status=400
                        )
...
class PostingOrderViewTest(TestCase):
    ...
    def test_get_list_action(self, mock_get_listview):
        mock_get_listview.return_value = [{"id": 1, "jobdesc": "Test posting order"}]
