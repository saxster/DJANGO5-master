...
class TicketManagerTest(TestCase):
    ...
    def test_send_ticket_mail(self):
        ...
        with patch.object(QueryRepository, "ticketmail") as mock_ticketmail:
            mock_ticketmail.return_value = [Mock()]

            result = Ticket.objects.send_ticket_mail(self.ticket.id)

            self.assertIsNotNone(result)
            mock_ticketmail.assert_called_once_with(self.ticket.id)
    ...
    def test_get_tickets_listview(self, mock_parse_params):
        ...
        request.session = {"assignedsites": [self.bt.id], "client_id": self.bt.id}
    ...
    def test_get_ticketlist_for_escalation(self):
        ...
        with patch.object(QueryRepository, "get_ticketlist_for_escalation") as mock_escalation:
            mock_escalation.return_value = [Mock()]
    ...
    def test_get_ticket_stats_for_dashboard(self):
        ...
        request.session = {"assignedsites": [self.bt.id], "client_id": self.bt.id}
        request.GET = {"from": "2024-01-01", "upto": "2024-12-31"}
    ...
    def test_get_events_for_calendar(self):
        ...
        request.session = {"bu_id": self.bt.id, "client_id": self.bt.id}
        request.GET = {
...
class ESCManagerTest(TestCase):
    ...
    def test_esc_manager_instance(self):
        ...
        self.assertTrue(
            hasattr(EscalationMatrix.objects, "handle_reminder_config_postdata")
        )
    ...
    def test_get_reminder_config_forppm(self):
        ...
        with patch.object(EscalationMatrix.objects, "filter") as mock_filter:
            ...
            result = EscalationMatrix.objects.get_reminder_config_forppm(
                self.job.id, fields
            )

            mock_filter.assert_called_with(
                escalationtemplate__tacode="JOB", job_id=self.job.id
            )
    ...
    def test_handle_reminder_config_postdata_create(self, mock_get_type_assist):
        ...
        with patch.object(EscalationMatrix.objects, "filter") as mock_filter_method:
            ...
            with patch.object(
                EscalationMatrix.objects, "create", return_value=mock_instance
            ) as mock_create:
                result = EscalationMatrix.objects.handle_reminder_config_postdata(
                    request
                )
    ...
    @patch("apps.y_helpdesk.managers.TypeAssist.objects.get")
    def test_handle_reminder_config_postdata_duplicate_record(
        self, mock_get_type_assist
    ):
        mock_type_assist = Mock()
        mock_get_type_assist.return_value = mock_type_assist
