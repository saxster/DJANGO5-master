...
class PeopleManager(TenantAwareManager, BaseUserManager):
    ...
    def get_people_modified_after(self, mdtz, siteid):
        ...
        qset = (
            ...
            .filter(Q(id=1) | Q(bu_id=siteid) & Q(mdtz__gte=mdtz))
            .values(*self.fields)
            .order_by("-mdtz")
        )
    ...
    def get_emergencycontacts(self, siteid, clientid):
        ...
        qset = (
            self.filter(
                bu_id=siteid, client_id=clientid, people_extras__isemergencycontact=True
            )
            .annotate(
                pmobno=Concat(
                    F("peoplename"),
                    V(":", output_field=models.CharField()),
                    Cast("mobno", output_field=models.CharField()),
                )
            )
            .values_list("pmobno", flat=True)
            .exclude(mobno=None)
        )
        return qset or self.none()
    ...
    def controlroomchoices(self, request):
        ...
        qset = (
            ...
            .annotate(text=Concat(F("peoplename"), V(" ("), F("peoplecode"), V(")")))
            .values_list("id", "text")
        )
        return qset or self.none()
    ...
    def get_people_for_posted_ppl_on_bu(self, request):
        qset = (
            self.filter(client_id=request.session["client_id"], enable=True)
            .exclude(Q(designation__tacode__in=["CR"]) | Q(worktype__tacode__in=["CR"]))
            .annotate(text=Concat(F("peoplename"), V(" ("), F("peoplecode"), V(")")))
            .values_list("id", "text")
        )
        return qset or self.none()
    ...
    def get_assigned_sites(self, clientid, peopleid):
        ...
        qset = (
            ...
            .annotate(text=Concat(F("buname"), V(" ("), F("bucode"), V(")")))
            .values_list("id", "text")
        )
    ...
    def get_people_pic(self, peopleid):
        ...
        qset = (
            self.filter(id=peopleid)
            .annotate(
                default_img_path=Concat(
                    V("youtility4_media/", output_field=models.CharField()),
                    Cast("peopleimg", output_field=models.CharField()),
                )
            )
            .values_list("default_img_path", named=True)
        )
    ...
    def get_siteincharge_emails(self, buid):
        ...
        grps = list(
            Pgbelonging.objects.annotate(groupid=Cast("pgroup_id", models.CharField()))
            .filter(
                assignsites_id=buid,
        ...
        return (
            self.filter(
                ...
                & Q(
                    Q(designation__tacode="SITEINCHARGE")
                    | Q(worktype__tacode="SITEINCHARGE")
                )
                & Q(isverified=True)
            ).values_list("email", flat=True)
            or []
    ...
    def get_peoples_at_site(self, request):
        ...
        sitegrp_ids = list(
            Pgbelonging.objects.filter(assignsites_id=bu_id).values_list(
                "pgroup_id", flat=True
            )
        )

        qset = (
            DeviceEventlog.objects.filter(
                Q(people__people_extras__assignsitegroup__in=sitegrp_ids)
                | Q(people__bu_id=bu_id),
            )
            .annotate(
                ...
                worktype=F("people__worktype__taname"),
            )
            .values(
                "gps",
                "mobno",
    ...
    def peoplechoices_for_pgroupform(self, request):
        ...
        qset = (
            ...
            .annotate(
                peopletext=Concat(F("peoplename"), V(" ("), F("peoplecode"), V(")"))
            )
            .values_list("id", "peopletext")
        )
        return qset or self.none()
    ...
    def getPeoplesForEscForm(self, request):
        ...
        qset = (
            qset.filter(peoplename__icontains=R["search"]) if R.get("search") else qset
        )
        qset = qset.annotate(
    ...
    def people_list_view(self, request, fields, related):
        ...
        if request.user.isadmin:
            qset = (
                self.filter(~Q(peoplecode="NONE"), client_id=S["client_id"])
...
class CapabilityManager(TenantAwareManager):
    ...
    def get_portletparentdata(self):
        return self.filter(
            cfor=self.pm.Capability.Cfor.PORTLET, parent__capscode="NONE"
        )
    ...
    def get_web_caps_for_client_orm(self):
        """
        Django ORM implementation to replace get_web_caps_for_client raw SQL.
        Returns capability hierarchy with depth and path information.
        """
        from apps.core.queries import TreeTraversal
        
        # Get all WEB capabilities
        all_caps = list(
            self.filter(cfor='WEB')
            .values('id', 'capscode', 'capsname', 'parent_id', 'cfor')
        )
        
        if not all_caps:
            return []
        
        # Build tree structure starting from root (id=1)
        ...
        for node in tree_data:
            # Find the original capability data
            cap_data = next(
                (cap for cap in all_caps if cap['id'] == node['id']),
                None
            )
            ...
            if cap_data:
                ...
                class CapabilityResult:
                    def __init__(self, data, depth, path, xpath):
                        self.id = data['id']
                        self.capscode = data['capscode']
                        self.capsname = data['capsname']
        ...
        # Sort by xpath to match PostgreSQL ordering
...
class PgblngManager(TenantAwareManager):
    ...
    def get_modified_after(self, mdtz, peopleid, buid):
        qset = (
            self.select_related(*self.related)
            .filter(Q(people_id=peopleid) | Q(bu_id=buid), mdtz__gte=mdtz)
            .values(*self.fields)
            .order_by("-mdtz")
        )
        return qset or self.none()
        ...
        if force or not qset:
            qset = (
                self.annotate(
                    ...
                    bu__buname=F("assignsites__buname"),
                    bucode=F("assignsites__bucode"),
                    buid=F("assignsites_id"),
                    solid=F("assignsites__solid"),
                )
                .select_related("assignsites", "identifier")
                .filter(pgroup_id=job["sgroup_id"])
    ...
    def get_assigned_sites_to_people(
        ...
        if people and people.isadmin:
            bu_ids = list(
                Bt.objects.get_all_sites_of_client(
                    clientid=people.client_id
                ).values_list("id", flat=True)
            )
        else:
            assigned_sitegroup_ids = (
                People.objects.filter(id=peopleid)
                .values("people_extras__assignsitegroup")
                .first()
            )
            # get sites assigned to groupids
            bu_ids = list(
                self.select_related("assignsites")
                .filter(
                    pgroup_id__in=assigned_sitegroup_ids[
                        "people_extras__assignsitegroup"
                    ]
                )
        ...
        if makechoice:
            return self.make_choices_of_sites(total_assigned_sites + [1])

        # for mobile service
        if forservice:
            return self.return_sites_for_service(
                total_assigned_sites,
                [
                    "buname",
                    "bucode",
                    "bu_id",
...
class PgroupManager(TenantAwareManager):
    ...
    def listview(self, request, fields, related, orderby, dir=None, qobjs=None):
        ...
        if not qobjs:
            objs = (
                ...
                .filter(identifier__tacode="SITEGROUP")
                .values(*fields)
                .order_by(f"{order}{orderby}")
            )
        else:
            objs = (
                self.select_related(*related)
                .filter(qobjs, identifier__tacode="SITEGROUP")
                .values(*fields)
                .order_by(f"{order}{orderby}")
            )
    ...
    def get_groups_modified_after(self, mdtz, buid):
        ...
        qset = (
            ...
            .filter(
                ...
                | Q(mdtz__gte=mdtz)
                & Q(bu_id=buid)
                & Q(identifier__tacode="PEOPLEGROUP")
            )
            .values(*self.fields)
        )
        return qset or None
    ...
    def get_assignedsitegroup_forclient(self, clientid, request):
        qset = self.filter(
            enable=True, client_id=clientid, identifier__tacode="SITEGROUP"
    ...
    def getGroupsForEscForm(self, request):
        ...
        qset = self.filter(
            bu_id=S["bu_id"], client_id=S["client_id"], identifier__tacode="PEOPLEGROUP"
        ).select_related("identifier", "bu", "client")
        qset = (
            qset.filter(groupname__icontains=R["search"]) if R.get("search") else qset
        )
        qset = qset.annotate(text=F("groupname")).values("id", "text")
        return qset or self.none()
