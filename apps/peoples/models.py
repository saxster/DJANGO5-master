...
def upload_peopleimg(instance, filename):
    try:
        ...
        logger.critical("upload_peopleimg(instance, filename)... FAILED", exc_info=True)
    else:
...
class SecureString(CharField):
    ...
    def from_db_value(self, value, expression, connection):
        ...
        if value and value != "":
            try:
                ...
                # If decryption fails, assume it's plain text (for migration compatibility)
                return value
        return value
    ...
    def get_prep_value(self, value):
        ...
        if value and value != "":
            try:
                # Check if it's already encrypted (base64-like pattern)
                ...
                # If encryption fails, return plain text for now
                return value
        return value
...
class People(AbstractBaseUser, PermissionsMixin, TenantAwareModel, BaseModel):
    ...
    def save(self, *args, **kwargs):
        ...
        # If we set any defaults, save again to persist them
        if needs_update:
            super().save(update_fields=['department', 'designation', 'peopletype', 'worktype', 'reportto'])
