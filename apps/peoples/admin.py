...
class PeopleResource(resources.ModelResource):
    ...
    def validations(self, row):
        ...
        if (
            ...
            raise ValidationError(
                f"Record with the se values already exist {row.values()}"
            )
...
class CapabilityAdmin(ImportExportModelAdmin):
    ...
    def get_queryset(self, request):
        return pm.Capability.objects.select_related("parent", "cuser", "muser").all()
