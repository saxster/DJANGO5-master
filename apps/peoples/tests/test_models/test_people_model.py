...
class TestPeopleModel:
    ...
    def test_people_str_representation(self, people_factory):
        ...
        person = people_factory(peoplename="John Doe", peoplecode="JD001")
    ...
    def test_people_authentication_integration(self, people_factory, test_password):
        ...
        # Test authentication
        authenticated_user = authenticate(username="authuser", password=test_password)
        assert authenticated_user == person
        assert authenticated_user.is_authenticated

    def test_people_unique_constraints(
        self, people_factory, test_client_bt, test_bu_bt
    ):
    ...
    def test_people_admin_flags(self, people_factory):
        ...
        admin_person = people_factory(
            peoplecode="ADMIN001", loginid="admin_user", isadmin=True, is_staff=True
        )

        regular_person = people_factory(
            peoplecode="REG001", loginid="regular_user", isadmin=False, is_staff=False
        )
    ...
    def test_people_reporting_hierarchy(self, people_factory):
        ...
        manager = people_factory(
            peoplecode="MGR001", peoplename="Manager", loginid="manager"
        )
    ...
    def test_people_verification_status(self, people_factory):
        ...
        unverified_person = people_factory(
            peoplecode="UNVER001", loginid="unverified", isverified=False
        )

        assert verified_person.isverified is True
        assert unverified_person.isverified is False

    def test_people_foreign_key_relationships(
        self, people_factory, test_client_bt, test_bu_bt
    ):
    ...
    def test_people_secure_string_fields(self, people_factory):
        ...
        person = people_factory(email="secure@example.com", mobno="9876543210")

        # Test that values are stored and encryption/decryption works
        ...
        with connection.cursor() as cursor:
            cursor.execute(
                "SELECT email, mobno FROM people WHERE id = %s",
                [person.id]
            )
            row = cursor.fetchone()
            stored_email, stored_mobno = row

            # Encrypted data should be different from plain text
            assert stored_email != "secure@example.com"
            assert stored_mobno != "9876543210"
            # Encrypted data should be base64-like strings
            assert len(stored_email) > len("secure@example.com")
            assert len(stored_mobno) > len("9876543210")
    ...
    def test_people_multi_tenant_isolation(self, people_factory):
        ...
        person1 = people_factory(
            peoplecode="TENANT1", loginid="tenant1_user", client=client1, bu=client1
        )
        person2 = people_factory(
            peoplecode="TENANT2", loginid="tenant2_user", client=client2, bu=client2
        )
