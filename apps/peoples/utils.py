...
def save_cuser_muser(instance, user, create=None):
    ...
    debug_logger.debug(
        f"while saving cuser and muser {instance.muser = } {instance.cuser = } {instance.mdtz = } {instance.cdtz = }"
    )
...
def save_client_tenantid(instance, user, session, client=None, bu=None):
    ...
    if hasattr(instance, "client_id"):
        client = (
            ...
            if instance.client_id in [1, None]
            else instance.client_id
        )
    if client is None:
        session.get("client_id")
...
def save_userinfo(instance, user, session, client=None, bu=None, create=True):
    ...
    if user.is_authenticated:
        try:
            ...
            instance.save()
            logger.info(
                f"while saving cdtz and mdtz id {instance.cdtz=} {instance.mdtz=}"
            )
    ...
    if client:
        if not web:
            web = Capability.objects.filter(
                capscode__in=client.bupreferences["webcapability"],
        ...
        if not mob:
            mob = Capability.objects.filter(
                capscode__in=client.bupreferences["mobilecapability"],
                cfor=Capability.Cfor.MOB,
                enable=True,
            ).values_list("capscode", "capsname")
            cache.set("mobcaps", mob, 30)
        if not portlet:
            portlet = Capability.objects.filter(
                capscode__in=client.bupreferences["portletcapability"],
                cfor=Capability.Cfor.PORTLET,
                enable=True,
            ).values_list("capscode", "capsname")
            cache.set("portletcaps", portlet, 30)
        if not report:
            report = Capability.objects.filter(
                capscode__in=client.bupreferences["reportcapability"],
                cfor=Capability.Cfor.REPORT,
                enable=True,
            ).values_list("capscode", "capsname")
            cache.set("reportcaps", report, 30)
        if not noc:
            noc = Capability.objects.filter(
                cfor=Capability.Cfor.NOC, enable=True
            ).values_list("capscode", "capsname")
...
def get_caps_choices(client=None, cfor=None, session=None, people=None):
    ...
    # Handle MOB and NOC cases directly without fetching web caps
    if cfor == Capability.Cfor.MOB:
        return (
            Capability.objects.select_related("parent")
            .filter(cfor=cfor, enable=True)
            .values_list("capscode", "capsname")
        )
    ...
    # For WEB capabilities, use the Django ORM query which already has caching
    debug_logger.debug("Getting web capabilities...")
    caps = get_query("get_web_caps_for_client")
    debug_logger.debug("Got capabilities from Django ORM query (with built-in caching)")
...
def get_choices_for_peoplevsgrp(request):
    ...
    return pm.People.objects.filter(bu__btid=site.btid).values_list(
        "people", "peoplename"
    )
