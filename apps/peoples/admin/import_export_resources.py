"""
Import/Export Resource classes for peoples models.

Provides:
    - PeopleResource: Create new people via import
    - PeopleResourceUpdate: Update existing people via import
    - GroupResource: Create new groups via import
    - GroupResourceUpdate: Update existing groups via import
    - GroupBelongingResource: Create group memberships via import
    - GroupBelongingResourceUpdate: Update group memberships via import
    - CapabilityResource: Create/update capabilities via import
"""
...
from apps.core.widgets import (
    BVForeignKeyWidget,
    TypeAssistDepartmentFKW,
    TypeAssistDesignationFKW,
    TypeAssistEmployeeTypeFKW,
    TypeAssistWorkTypeFKW,
    TypeAssistDepartmentFKWUpdate,
    TypeAssistDesignationFKWUpdate,
    TypeAssistEmployeeTypeFKWUpdate,
...
# =============================================================================
# PEOPLE RESOURCES (CREATE & UPDATE)
# =============================================================================


class PeopleResource(resources.ModelResource):
    """Import/Export resource for creating new People records."""
    ...
    DateOfBirth = fields.Field(
        column_name="Date of Birth*", attribute="dateofbirth", widget=wg.DateWidget()
    )

    DateOfJoin = fields.Field(
        column_name="Date of Join*", attribute="dateofjoin", widget=wg.DateWidget()
    )

    date_of_release = fields.Field(
        column_name="Date of Release", attribute="dateofreport", widget=wg.DateWidget()
    )
    ...
    isemergencycontact = fields.Field(
        widget=wg.BooleanWidget(), default=False, column_name="Emergency Contact"
    )
    ...
    blacklist = fields.Field(
        widget=wg.BooleanWidget(), default=False, column_name="Blacklist"
    )
    ...
    class Meta:
        ...
        fields = [
            "Code", "Name", "LoginId", "Designation", "Department", "MobNo", "Email",
            "deviceid", "Site", "DateOfJoin", "date_of_release", "DateOfBirth",
            "Gender", "PeopleType", "WorkType", "Enable", "Client",
            "isemergencycontact", "alertmails", "mobilecaps", "reportcaps", "webcaps",
    ...
    def before_import_row(self, row, **kwargs):
        ...
        self._mobilecaps = (
            clean_array_string(row["Mobile Capability"])
            if row.get("Mobile Capability")
            else []
        )
        self._reportcaps = (
            clean_array_string(row["Report Capability"])
            if row.get("Report Capability")
            else []
        )
        self._webcaps = (
            clean_array_string(row["Web Capability"])
            if row.get("Web Capability")
            else []
        )
        self._portletcaps = (
            clean_array_string(row["Portlet Capability"])
            if row.get("Portlet Capability")
            else []
        )
        ...
        self._userfor = (
            row.get("User For*").capitalize() if row.get("User For*") else "Mobile"
        )
        row["Mob No*"] = str(row["Mob No*"])

    def before_save_instance(self, instance, row, **kwargs):
        """Set extras and save common metadata."""
    ...
    def validations(self, row):
        ...
        if row["User For*"] in ["", None]:
            raise ValidationError("User For* is required field")
        ...
        # Unique record check
        if (
            pm.People.objects.select_related()
            .filter(
...
class PeopleResourceUpdate(resources.ModelResource):
    """Import/Export resource for updating existing People records."""

    # Field definitions (similar to PeopleResource but for updates)
    Client = fields.Field(
        column_name="Client",
        attribute="client",
        widget=wg.ForeignKeyWidget(om_client.Bt, "bucode"),
    ...
    Code = fields.Field(attribute="peoplecode", column_name="Code")
    userfor = fields.Field(
        column_name="User For", attribute="people_extras.userfor", default="Mobile"
    )
    deviceid = fields.Field(attribute="deviceid", column_name="Device Id", default=-1)
    Name = fields.Field(attribute="peoplename", column_name="Name")
    LoginId = fields.Field(attribute="loginid", column_name="Login ID")
    ...
    Enable = fields.Field(widget=wg.BooleanWidget(), attribute="enable", default=True)
    isemergencycontact = fields.Field(
        column_name="Is Emergency Contact",
        attribute="people_extras.isemergencycontact",
        widget=wg.BooleanWidget(),
        default=False,
    )
    alertmails = fields.Field(
        column_name="Alert Mails",
        attribute="people_extras.alertmails",
        widget=wg.BooleanWidget(),
        default=False,
    )
    mobilecaps = fields.Field(
        column_name="Mobile Capability",
        attribute="people_extras.mobilecapability",
        default=[],
    )
    reportcaps = fields.Field(
        column_name="Report Capability",
        attribute="people_extras.reportcapability",
        default=[],
    )
    webcaps = fields.Field(
        column_name="Web Capability",
        attribute="people_extras.webcapability",
        default=[],
    )
    currentaddr = fields.Field(
        column_name="Current Address",
        attribute="people_extras.currentaddress",
        default="",
    )
    portletcaps = fields.Field(
        column_name="Portlet Capability",
        attribute="people_extras.portletcapability",
        default=[],
    )
    blacklist = fields.Field(
        column_name="Blacklist",
        attribute="people_extras.blacklist",
        widget=wg.BooleanWidget(),
        default=False,
    )
    ...
    class Meta:
        ...
        report_skipped = True
        fields = [
            "ID", "Code", "Name", "LoginId", "Designation", "Department", "MobNo",
            "Email", "deviceid", "Site", "DateOfJoin", "date_of_release",
            "DateOfBirth", "Gender", "PeopleType", "WorkType", "Enable", "Client",
            "isemergencycontact", "alertmails", "mobilecaps", "reportcaps", "webcaps",
    ...
    def before_import_row(self, row, **kwargs):
        """Clean and validate row data before update."""
        self.validations(row)
    ...
    def before_save_instance(self, instance, using_transactions, dry_run):
        """Update extras and save common metadata."""
        instance.email = instance.email.lower()
...
# =============================================================================
# GROUP RESOURCES (CREATE & UPDATE)
# =============================================================================


class GroupResource(resources.ModelResource):
    """Import/Export resource for creating new Group records."""
    ...
    def before_import_row(self, row, row_number=None, **kwargs):
        ...
        # Unique record check
        if (
            pm.Pgroup.objects.select_related()
            .filter(
                groupname=row["Name*"],
    ...
    def before_save_instance(self, instance, using_transactions=None, dry_run=False):
        """Save common metadata."""
        utils.save_common_stuff(self.request, instance, self.is_superuser)


class GroupResourceUpdate(resources.ModelResource):
    """Import/Export resource for updating existing Group records."""
    ...
    def before_import_row(self, row, row_number=None, **kwargs):
        """Validate row data before update."""
        if "Name" in row:
        ...
        # Check record exists
        if not pm.Pgroup.objects.filter(id=row["ID*"]).exists():
            raise ValidationError(
                f"Record with these values not exist: ID - {row['ID*']}"
            )
...
# =============================================================================
# GROUP BELONGING RESOURCES (CREATE & UPDATE)
# =============================================================================


class GroupBelongingResource(resources.ModelResource):
    """Import/Export resource for creating new Group Belonging records."""
    ...
    GROUP = fields.Field(
        ...
        default=get_or_create_none_pgroup,
    )
    PEOPLE = fields.Field(
    ...
    def before_import_row(self, row, row_number=None, **kwargs):
        """Validate row data before import."""
        if row.get("Group Name*") in ["", "NONE", None]:
            raise ValidationError({"Group Name": "This field is required"})

        if row.get("Of Site") in ["", "NONE", None] and row.get("Of People") in [
            "", "NONE", None,
        ]:
            raise ValidationError(
                "Either Site or People should be set, both cannot be None"
            )

        # Unique record check
        if (
            pm.Pgbelonging.objects.select_related()
            .filter(
                people__peoplecode=row["Of People"],
                pgroup__groupname=row["Group Name*"],
                client__bucode=row["Client*"],
                assignsites__bucode=row["Of Site"],
                bu__bucode=row["Site*"],
            )
            .exists()
        ):
...
class GroupBelongingResourceUpdate(resources.ModelResource):
    """Import/Export resource for updating existing Group Belonging records."""
...
class CapabilityResource(resources.ModelResource):
    """Import/Export resource for Capability model."""
    ...
    class Meta:
        ...
        report_skipped = True
        import_id_fields = ("Code",)
        fields = ("Code", "Name", "cfor", "parent", "Client", "BV", "tenant")
    ...
    def before_save_instance(self, instance, row=None, **kwargs):
        """Uppercase code and save common metadata."""
    ...
    def skip_row(self, instance, original, row, import_validation_errors=None):
        ...
        return pm.Capability.objects.filter(
            capscode=instance.capscode, cfor=instance.cfor
        ).exists()
