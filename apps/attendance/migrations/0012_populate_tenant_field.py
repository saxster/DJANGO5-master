"""
Data migration to populate tenant field for existing attendance records.
...
def populate_tenant_field(apps, schema_editor):
    ...
    # Populate tenant
    updated_count = PeopleEventlog.objects.filter(tenant__isnull=True).update(
        tenant=default_tenant
    )
    print(f"Updated {updated_count} PeopleEventlog records with default tenant")
...
def reverse_populate_tenant(apps, schema_editor):
    ...
    try:
        ...
        reverted_count = PeopleEventlog.objects.filter(tenant=default_tenant).update(tenant=None)
        print(f"Reverted {reverted_count} PeopleEventlog records to NULL tenant")
...
class Migration(migrations.Migration):
    ...
    dependencies = [
        ('attendance', '0011_add_tenant_indexes'),
