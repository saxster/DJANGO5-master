...
class PeopleEventlog(BaseModel, TenantAwareModel):
    ...
    peventlogextras = models.JSONField(
        _("peventlogextras"), encoder=DjangoJSONEncoder, default=peventlog_json
    )
    ...
    def startlocation_display(self):
        """Returns a human-readable format of start location"""
        if self.startlocation:
            coords = self.startlocation.coords
    ...
    def endlocation_display(self):
        """Returns a human-readable format of end location"""
        if self.endlocation:
            coords = self.endlocation.coords
            return f"Lat: {coords[1]:.6f}, Lng: {coords[0]:.6f}"
        return ""
