...
class TestPeopleEventlogModel:
    ...
    def test_people_eventlog_str_representation(self, people_eventlog_factory):
        ...
        eventlog = people_eventlog_factory(remarks="Daily attendance check-in")
    ...
    def test_people_eventlog_transport_mode_choices(
        self, people_eventlog_factory, transport_modes
    ):
        ...
        for mode in transport_modes[:3]:  # Test first 3 to avoid too many objects
            eventlog = people_eventlog_factory(
                transportmodes=[mode], deviceid=f"DEVICE_{mode}"
            )
    ...
    def test_people_eventlog_multiple_transport_modes(self, people_eventlog_factory):
        ...
        modes = ["BUS", "TRAIN"]
        eventlog = people_eventlog_factory(transportmodes=modes)
    ...
    def test_people_eventlog_gps_locations(
        self, people_eventlog_factory, gps_coordinates
    ):
    ...
    def test_people_eventlog_json_fields(self, people_eventlog_factory):
        ...
        eventlog = people_eventlog_factory(
            peventlogextras=custom_extras, geojson=custom_geojson
        )
    ...
    def test_people_eventlog_face_recognition_flags(self, people_eventlog_factory):
        ...
        eventlog_with_face = people_eventlog_factory(
            facerecognitionin=True, facerecognitionout=True
        )
        ...
        eventlog_without_face = people_eventlog_factory(
            facerecognitionin=False, facerecognitionout=False, deviceid="NO_FACE_DEVICE"
        )
    ...
    def test_people_eventlog_financial_fields(self, people_eventlog_factory):
        ...
        eventlog = people_eventlog_factory(expamt=150.75, distance=25.5)
    ...
    def test_people_eventlog_device_tracking(self, people_eventlog_factory):
        ...
        eventlog = people_eventlog_factory(
            deviceid="ANDROID_12345", accuracy=89.5, reference="MOBILE_CHECKIN"
        )
    ...
    def test_people_eventlog_relationships(
        self, people_eventlog_factory, test_people, test_client_bt, test_bu_bt
    ):
    ...
    def test_people_eventlog_shift_assignment(
        self, people_eventlog_factory, test_shift
    ):
        """Test PeopleEventlog shift assignment"""
        eventlog = people_eventlog_factory(shift=test_shift)
    ...
    def test_people_eventlog_geofence_integration(
        self, people_eventlog_factory, test_geofence
    ):
        """Test PeopleEventlog geofence integration"""
        eventlog = people_eventlog_factory(geofence=test_geofence)
    ...
    def test_people_eventlog_bulk_operations(
        self, people_eventlog_factory, test_people
    ):
        ...
        today_events = PeopleEventlog.objects.filter(
            datefor=date.today(), people=test_people
        )
        assert today_events.count() == 1

        # Test bulk update
        PeopleEventlog.objects.filter(deviceid__startswith="BULK_DEVICE").update(
            remarks="Bulk updated remark"
        )

        # Verify bulk update
        updated_events = PeopleEventlog.objects.filter(
            deviceid__startswith="BULK_DEVICE", remarks="Bulk updated remark"
        )
    ...
    def test_people_eventlog_date_filtering(self, people_eventlog_factory):
        ...
        today_event = people_eventlog_factory(datefor=today, deviceid="TODAY_DEVICE")
        yesterday_event = people_eventlog_factory(
            datefor=yesterday, deviceid="YESTERDAY_DEVICE"
        )
    ...
    def test_people_eventlog_tenant_isolation(self, people_eventlog_factory):
        ...
        event1 = people_eventlog_factory(
            client=client1, bu=client1, deviceid="CLIENT1_DEVICE"
        )
        event2 = people_eventlog_factory(
            client=client2, bu=client2, deviceid="CLIENT2_DEVICE"
        )
    ...
    def test_people_eventlog_performance_fields(self, people_eventlog_factory):
        ...
        eventlog = people_eventlog_factory(
            accuracy=95.7, distance=12.5, duration=300, expamt=75.50  # 5 hours
        )
        ...
        if eventlog.distance and eventlog.duration:
            ...
            speed_kmh = (
                (eventlog.distance / (eventlog.duration / 60))
                if eventlog.duration > 0
                else 0
            )
            assert speed_kmh >= 0
