...
class TestTrackingModel:
    ...
    def test_tracking_identifier_choices(self, tracking_factory):
        ...
        for identifier in identifiers:
            tracking = tracking_factory(
                identifier=identifier, deviceid=f"DEVICE_{identifier}"
            )
    ...
    def test_tracking_gps_location(self, tracking_factory, gps_coordinates):
        ...
        tracking = tracking_factory(gpslocation=location)
        ...
        mumbai_tracking = tracking_factory(
            gpslocation=gps_coordinates["mumbai_office"], deviceid="MUMBAI_DEVICE"
        )
    ...
    def test_tracking_device_identification(self, tracking_factory):
        ...
        tracking = tracking_factory(
            deviceid="MOBILE_ANDROID_123456", transportmode="CAR"
        )
    ...
    def test_tracking_time_fields(self, tracking_factory):
        ...
        tracking = tracking_factory(receiveddate=received_time)
    ...
    def test_tracking_transport_modes(self, tracking_factory):
        ...
        transport_modes = ["BIKE", "CAR", "BUS", "TRAIN", "WALKING", "NONE"]

        for mode in transport_modes:
            tracking = tracking_factory(transportmode=mode, deviceid=f"DEVICE_{mode}")
            assert tracking.transportmode == mode

    def test_tracking_reference_field(self, tracking_factory):
        """Test Tracking reference field"""
        tracking = tracking_factory(
            reference="EXTERNAL_TOUR_REF_001", identifier="EXTERNALTOUR"
        )
    ...
    def test_tracking_people_relationship(self, tracking_factory, test_people):
        ...
        tracking = tracking_factory(people=test_people)
    ...
    def test_tracking_bulk_operations(self, tracking_factory, test_people):
        ...
        # Test bulk filtering
        bus_trackings = Tracking.objects.filter(transportmode="BUS", people=test_people)
        train_trackings = Tracking.objects.filter(
            transportmode="TRAIN", people=test_people
        )
        ...
        # Test bulk update
        Tracking.objects.filter(deviceid__startswith="BULK_TRACK").update(
            identifier="CONVEYANCE"
        )

        # Verify bulk update
        updated_trackings = Tracking.objects.filter(
            deviceid__startswith="BULK_TRACK", identifier="CONVEYANCE"
        )
    ...
    def test_tracking_location_based_queries(self, tracking_factory, gps_coordinates):
        ...
        bangalore_tracking = tracking_factory(
            gpslocation=gps_coordinates["bangalore_office"], deviceid="BLR_DEVICE"
        )
        mumbai_tracking = tracking_factory(
            gpslocation=gps_coordinates["mumbai_office"], deviceid="MUM_DEVICE"
        )

        # Test basic location existence
        bangalore_trackings = Tracking.objects.filter(
            gpslocation__isnull=False, deviceid="BLR_DEVICE"
        )
        mumbai_trackings = Tracking.objects.filter(
            gpslocation__isnull=False, deviceid="MUM_DEVICE"
        )
    ...
    def test_tracking_time_range_queries(self, tracking_factory):
        ...
        recent_tracking = tracking_factory(receiveddate=now, deviceid="RECENT_DEVICE")
        old_tracking = tracking_factory(receiveddate=hour_ago, deviceid="OLD_DEVICE")
    ...
    def test_tracking_str_representation(self, tracking_factory):
        ...
        tracking = tracking_factory(deviceid="TEST_DEVICE_STR")
    ...
    def test_tracking_tour_workflow(self, tracking_factory):
        ...
        external_tracking = tracking_factory(
            identifier="EXTERNALTOUR", reference="EXT_TOUR_001", deviceid="EXT_DEVICE"
        )

        # Internal tour tracking
        internal_tracking = tracking_factory(
            identifier="INTERNALTOUR", reference="INT_TOUR_001", deviceid="INT_DEVICE"
        )
    ...
    def test_tracking_site_visit_workflow(self, tracking_factory):
        ...
        tracking = tracking_factory(identifier="SITEVISIT", reference="SITE_VISIT_001")
    ...
    def test_tracking_performance_queries(self, tracking_factory):
        ...
        tracking_count = Tracking.objects.filter(
            deviceid__startswith="PERF_DEVICE", identifier="TRACKING"
        ).count()
        conveyance_count = Tracking.objects.filter(
            deviceid__startswith="PERF_DEVICE", identifier="CONVEYANCE"
        ).count()
