...
def test_client_bt():
    ...
    return Bt.objects.create(
        bucode="ATTCLIENT", buname="Attendance Test Client", enable=True
    )
...
def test_bu_bt():
    """Create test BU"""
    return Bt.objects.create(bucode="ATTBU", buname="Attendance Test BU", enable=True)
...
@pytest.fixture
def people_eventlog_factory(
    test_people, test_client_bt, test_bu_bt, test_peventtype, test_shift, test_geofence
):
...
def tracking_factory(test_people):
    ...
    def _create_tracking(people=None, **kwargs):
        defaults = {
            "people": people or test_people,
...
def sample_attendance_data():
    ...
    return {
        ...
        "punchouttime": (timezone.now() + timedelta(hours=8)).strftime(
            "%d-%b-%Y %H:%M:%S"
        ),
