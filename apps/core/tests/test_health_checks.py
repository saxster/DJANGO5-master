...
class TestHealthCheckManager:
    ...
    def test_run_all_checks_degraded(self):
        ...
        manager.register_check(
            "critical_healthy", healthy_critical_check, critical=True
        )
        manager.register_check(
            "non_critical_fail", failing_non_critical_check, critical=False
        )
...
class TestPostgreSQLFunctionsCheck:
    ...
    def test_check_postgresql_functions_all_available(self, mock_cursor):
        ...
        # Mock all functions as existing (only the 3 functions that actually exist)
        mock_cursor_instance.fetchone.side_effect = [
    ...
    def test_check_postgresql_functions_missing(self, mock_cursor):
        ...
        # Mock some functions as missing (only the 3 functions that actually exist)
        mock_cursor_instance.fetchone.side_effect = [
...
class TestTaskQueueHealthCheck:
    ...
    @patch("django.db.connection.cursor")
    def test_check_task_queue_success(self, mock_cursor):
