...
class UserFriendlyResource(resources.ModelResource):
    ...
    def _handle_multiple_results_error(self, error_str, row):
        """Handle 'get() returned more than one' errors."""
        import re
    ...
    def _handle_does_not_exist_error(self, error_str, row):
        """Handle 'DoesNotExist' and 'matching query does not exist' errors."""
        if "Bt matching query does not exist" in error_str:
            return "Site or Client not found. Please ensure they exist in the system."
        if "Asset matching query does not exist" in error_str:
        ...
        if "QuestionSet matching query does not exist" in error_str:
        ...
        if "Job matching query does not exist" in error_str:
            belongs_to = row.get('Belongs To*', 'Unknown') if row else 'Unknown'
        ...
        if "People matching query does not exist" in error_str:
        ...
        if "TypeAssist matching query does not exist" in error_str:
            return self._handle_type_assist_not_found(row)
        return "Referenced record not found. Please check your data."

    def _handle_type_assist_not_found(self, row):
        """Handle TypeAssist not found errors."""
        if row:
            type_fields = {
        ...
        return "User Defined Type not found. Please ensure the type exists in the system."

    def _handle_integrity_error(self, error_str, row):
        """Handle IntegrityError exceptions."""
        if "duplicate key value violates unique constraint" not in error_str:
    ...
    def _handle_value_error(self, error_str):
        ...
        if "not found" in error_str and "Please ensure" in error_str:
            return error_str
    ...
    def format_error_message(self, error, row=None, field=None):
        ...
        if "get() returned more than one" in error_str:
            return self._handle_multiple_results_error(error_str, row)

        # Handle DoesNotExist errors
        if "DoesNotExist" in error_str or "matching query does not exist" in error_str:
            return self._handle_does_not_exist_error(error_str, row)

        # Handle IntegrityError
        if "IntegrityError" in error_str:
            return self._handle_integrity_error(error_str, row)

        # Handle ValueError
        if "ValueError" in error_str:
            return self._handle_value_error(error_str)
        ...
        if "Permission denied" in error_str or "not allowed" in error_str:
