...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            "--tenant-id", type=int, default=1, help="Tenant ID for cache operations"
        )
        parser.add_argument(
            "--dry-run",
            action="store_true",
    ...
    def show_stats(self, cache):
        ...
        if hasattr(cache, "get_stats"):
            ...
            self.stdout.write(
                f"Average data size: {stats['avg_data_size_bytes']} bytes"
            )
    ...
    def clear_cache(self, cache, dry_run):
        ...
        cache.clear()
        self.stdout.write(self.style.SUCCESS("✅ Cleared all Select2 cache entries"))
    ...
    def test_cache(self, cache):
        ...
        if hasattr(cache, "set_many"):
            ...
            if not failed_keys:
                ...
                self.stdout.write(
                    self.style.ERROR(f"❌ Batch set failed for: {failed_keys}")
                )
