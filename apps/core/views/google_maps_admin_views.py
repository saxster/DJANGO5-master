...
class GoogleMapsAdminDashboard(View):
    ...
    def get(self, request):
        ...
        try:
            ...
            logger.error(f"Failed to load Google Maps dashboard: {str(e)}")
...
def google_maps_stats_api(request):
    ...
    try:
        ...
        logger.error(f"Failed to get Google Maps stats: {str(e)}")
        return JsonResponse({
            'status': 'error',
            'message': str(e)
        }, status=500)
...
def google_maps_metrics_api(request):
    ...
    try:
        ...
        logger.error(f"Failed to get Google Maps metrics: {str(e)}")
        return JsonResponse({
            'status': 'error',
            'message': str(e)
        }, status=500)
...
def google_maps_clear_cache_api(request):
    ...
    try:
        ...
        logger.error(f"Failed to clear Google Maps cache: {str(e)}")
        return JsonResponse({
            'status': 'error',
            'message': str(e)
        }, status=500)
...
def google_maps_test_connection(request):
    ...
    try:
        ...
        logger.error(f"Google Maps connection test failed: {str(e)}")
        return JsonResponse({
            'status': 'error',
...
def google_maps_export_metrics(request):
    ...
    try:
        ...
        logger.error(f"Failed to export Google Maps metrics: {str(e)}")
        return JsonResponse({
            'status': 'error',
            'message': str(e)
        }, status=500)
...
def google_maps_config_info(request):
    ...
    try:
        ...
        logger.error(f"Failed to get Google Maps config info: {str(e)}")
        return JsonResponse({
            'status': 'error',
            'message': str(e)
        }, status=500)