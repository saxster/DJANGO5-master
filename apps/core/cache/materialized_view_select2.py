...
class MaterializedViewSelect2Cache(BaseCache):
            ...
            with transaction.atomic():
                with connection.cursor() as cursor:
                    ...
                    try:
                        ...
                        logger.info(
                            f"Select2 cache table {self._table_name} initialized"
                        )
    ...
    def _get_from_materialized_view(
        self, mv_name: str, tenant_id: int = None
    ) -> Optional[Any]:
    ...
    def get_many(
        ...
        for key in keys:
            mv_name = self._is_materialized_view_candidate(key)
            if mv_name:
                mv_data = self._get_from_materialized_view(
                    mv_name, self._get_tenant_id()
                )
                if mv_data:
                    results[key] = mv_data
                else:
        ...
        try:
            with connection.cursor() as cursor:
                ...
                stats = {
                    ...
                    "avg_data_size_bytes": int(result[3])
    ...
    def cleanup_expired(self) -> int:
        ...
        try:
            with connection.cursor() as cursor:
                ...
                if deleted_count > 0:
                    logger.info(
                        f"Cleaned up {deleted_count} expired Select2 cache entries"
                    )
