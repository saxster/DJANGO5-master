...
def safe_json_parse(
    json_string: str, fallback: Optional[Dict] = None
) -> Dict[str, Any]:
    """
...
def safe_json_parse_request_params(
    request_get, param_name: str = "params"
) -> Dict[str, Any]:
    """
    ...
    if "from" in parsed:
        try:
            ...
            logger.warning(
                f"Invalid 'from' date format: {parsed['from']}, using default"
            )
