...
class PointScalar(Scalar):
    ...
    def parse_literal(node):
        if node is None:
            return None
        return Point(
            float(node.get("longitude")), float(node.get("latitude")), srid=4326
        )
    ...
    def parse_value(value):
        if value is None:
            return None
        return Point(
            float(value.get("longitude")), float(value.get("latitude")), srid=4326
        )
...
class LineStringScalar(Scalar):
    ...
    def serialize(linestring):
        if linestring is None:
            return None
        return [
            {"latitude": point[1], "longitude": point[0]} for point in linestring.coords
        ]
...
class PolygonScalar(Scalar):
    ...
    def serialize(polygon):
        ...
        return [
            [{"latitude": point[1], "longitude": point[0]} for point in ring.coords]
            for ring in polygon
        ]
