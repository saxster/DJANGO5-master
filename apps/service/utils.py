...
def write_file_to_dir(filebuffer, uploadedfilepath):
    ...
    if hasattr(filebuffer, "read"):
        ...
        filebuffer, list
    ):  # Check if filebuffer is a list of integers (bytes) - kept for backward compatibility
        # Convert list of integers to bytes
        content = bytes(filebuffer)
    else:
        raise ValueError(f"Unsupported filebuffer type: {type(filebuffer)}")
...
def update_adhoc_record(scheduletask, jobneedrecord, details):
    ...
    log.info(
        f'record after updation {pformat(Jobneed.objects.filter(id =scheduletask["id"]).values())}'
    )
...
def perform_uploadattachment(file, record, biodata):
    ...
    log.info(
        f"Length file_buffer: '{len(file_buffer)}' \n'onwername':{onwername}, \nownerid: '{ownerid}' \npeopleid: '{peopleid}' \npath: {path} \nhome_dir: '{home_dir}' \nfilepath: '{filepath}' \nuploadfile: '{uploadfile}'"
    )
    try:
        with transaction.atomic(using=db):
    ...
    try:
        ...
        if hasattr(eobj, "peventtype") and eobj.peventtype.tacode in [
            ...
            log.warning(
                f"face recognition status {results.state} and {results} and task_id={results.task_id}"
            )
    except Exception as e:
