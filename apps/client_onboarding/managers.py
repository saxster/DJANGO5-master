...
class BtManager(models.Manager):
    ...
    def get_all_bu_of_client(self, clientid, type="array"):
        ...
        if type not in ("array", "text", "jsonb"):
            type = "array"  # Default to safe value

        # Import here to avoid circular import
        from .bt_manager_orm import BtManagerORM
        # Use Django ORM implementation
        return BtManagerORM.get_all_bu_of_client(clientid, type)
    ...
    def get_all_sites_of_client(self, clientid):
        ...
        return (
            self.select_related().filter(id__in=all_buids, identifier__tacode="SITE")
            or self.none()
        )
    ...
    def find_site(self, clientid, sitecode):
        ...
        """
        qset = self.filter(
            Q(identifier__tacode="SITE")
            & Q(bucode=sitecode)
            & ~Q(parent__id=-1)
            & Q(id__in=self.get_all_bu_of_client(clientid))
        )
    ...
    def get_sitelist_web(self, clientid, peopleid):
        ...
        from .bt_manager_orm import BtManagerORM
        # Use Django ORM implementation
        result = BtManagerORM.get_sitelist_web(clientid, peopleid)
        return result or self.none()
    ...
    def get_whole_tree(self, clientid):
        ...
        from .bt_manager_orm import BtManagerORM
        # Use Django ORM implementation
        return BtManagerORM.get_whole_tree(clientid)
    ...
    def getsitelist(self, clientid, peopleid):
        ...
        try:
            ...
            if p.isadmin:
                ...
                qset = (
                    bus.annotate(bu_id=F("id"))
                    .filter(identifier__tacode="SITE")
                    .values(
                        "bu_id",
                        "bucode",
                        "butype_id",
    ...
    #     parentid = (
    #         -1 if request.GET.get("parentid") == "None" else request.GET.get("parentid")
    #     )
    #     # Import here to avoid circular import
    #     from .bt_manager_orm import BtManagerORM
    #     # Use Django ORM implementation
    #     buids = BtManagerORM.get_whole_tree(parentid)
    #     qset = self.filter(id__in=buids).select_related(
    #         "identifier", "parent"
    #     )
    #     qset = qset.filter(buname__icontains=search_term) if search_term else qset
    #     qset = (
    #         qset.annotate(
    #             text=Concat(F("buname"), V(" ("), F("identifier__tacode"), V(")"))
    #         )
    #         .exclude(identifier__tacode="SITE")
    #         .values("id", "text")
    #     )
    #     return qset or self.none()
    ...
    def get_bus_idfs(self, R, request, idf=None):
        ...
        from .bt_manager_orm import BtManagerORM
        # Use Django ORM implementation
        bu_ids = BtManagerORM.get_whole_tree(S["client_id"])
        fields = R.getlist("fields[]")
        qset = (
            self.filter(
                ~Q(bucode__in=("NONE", "SPS", "YTPL")),
                identifier__tacode=idf,
                enable=True,
                id__in=bu_ids,
            )
            .select_related("parent", "identifier")
            .annotate(buid=F("id"), gps=AsGeoJSON("gpslocation"))
            .values(*fields)
        )
        idfs = (
            self.filter(
                ~Q(identifier__tacode="NONE"),
                ~Q(bucode__in=("NONE", "SPS", "YTPL")),
                id__in=bu_ids,
            )
            .order_by("identifier__tacode")
            .distinct("identifier__tacode")
            .values("identifier__tacode")
        )
        return qset, idfs

    def get_client_list(self, fields, related):
        qset = (
            self.filter(identifier__tacode="CLIENT")
    ...
    def handle_bupostdata(self, request):
        ...
        r = {"enable": R["enable"] == "1"}
        PostData = {
            "bucode": R["bucode"].upper(),
            "buname": R["buname"],
            "parent_id": R["parent"],
            "identifier_id": R["identifier"],
            "enable": r["enable"],
            "cuser": request.user,
            "muser": request.user,
    ...
    def handle_adminspostdata(self, request):
        ...
        PostData = {
            "peoplecode": R["peoplecode"].upper(),
            "peoplename": R["peoplename"],
            "bu_id": 1,
            "loginid": R["loginid"],
        ...
        if not utils.verify_mobno(R["mobno"]):
            return {
                "data": list(self.none()),
                "fieldErrors": [
                    {"name": "mobno", "status": "Please Enter Correct Mobile Number!"}
                ],
            }
        if not utils.verify_emailaddr(R["email"]):
            return {
                "data": list(self.none()),
                "fieldErrors": [
                    {"name": "email", "status": "Please Enter Correct Email Address!"}
                ],
            }

        if R["action"] == "create":
            if pm.People.objects.filter(
                peoplecode=R["peoplecode"].upper(), client_id=S["client_id"]
            ).exists():
                return {
                    "data": list(self.none()),
                    "error": "People code already exists",
                }
            ...
            if pm.People.objects.filter(
                email=R["email"], client_id=S["client_id"]
            ).exists():
    ...
    def get_listbus(self, request):
        ...
        if request.GET.get("id") == "None":
            return self.none()
        qset = (
            self.filter(identifier__tacode="SITE")
            .exclude(bucode__in=["NONE", "YTPL"])
            .distinct()
            .values(
                "id",
                "bucode",
    ...
    def get_listadmins(self, request):
        ...
        qset = (
            ...
            .exclude(peoplecode__in=["NONE", "SUPERADMIN"])
            .distinct()
            .values(
                "peoplecode",
                "peoplename",
    ...
    def get_sample_data(self):
        ...
        annotated_queryset = self.annotate(
            createdTime=ExpressionWrapper(
                F("cdtz") + timedelta(minutes=330), output_field=models.DateTimeField()
            )
        )
        # Convert the DateTimeField to a string
        annotated_queryset = (
            annotated_queryset.annotate(
                createdTimeStr=Cast(F("createdTime"), output_field=models.CharField())
            )
            .values(
                "id",
                "solid",
    ...
    def handle_device_limits_post(self, data):
        if data.get("client_id") and data["client_id"] != "None":
            if obj := self.filter(id=data["client_id"]).first():
                obj.bupreferences["no_of_devices_allowed"] = data[
    ...
    def handle_user_limits_post(self, data):
        if data.get("client_id") and data["client_id"] != "None":
            if obj := self.filter(id=data["client_id"]).first():
                obj.bupreferences["no_of_users_allowed_web"] = data[
                    "no_of_users_allowed_web"
                ]
                obj.bupreferences["no_of_users_allowed_mob"] = data[
                    "no_of_users_allowed_mob"
                ]
                obj.bupreferences["no_of_users_allowed_both"] = data[
    ...
    def get_sitecontract_details(self, request):
        ...
        S = request.session
        qset = (
            self.filter(id=S["bu_id"], enable=True)
            .values(
                "bupreferences__total_people_count",
                "bupreferences__contract_designcount",
            )
            .first()
        )
        ...
        if total_count and design_count is not None:
            ...
            for key, value in design_count.items():
                desgn_qset = (
                    TypeAssist.objects.filter(id=int(key)).values("tacode").first()
                )
...
class TypeAssistManager(models.Manager):
    ...
    def get_typeassist_modified_after(self, mdtz, clientid):
        ...
        qset = (
            ...
            .filter(
                Q(mdtz__gte=mdtz)
                & (
                    Q(client_id__in=[clientid])
                    | Q(cuser__is_superuser=True)
                    | Q(cuser__peoplecode="NONE")
                )
                & Q(enable=True)
            )
            .values(*self.fields)
    ...
    def load_identifiers(self, request):
        search_term = request.GET.get("search")
        qset = (
            self.filter(tatype__tacode="BVIDENTIFIER")
            .annotate(text=F("taname"))
            .exclude(taname="Client")
            .values("text", "id")
            .distinct()
        )
        qset = (
            qset.filter(taname__icontains=search_term, tatype__tacode="BVIDENTIFIER")
            if search_term
            else qset
        )
        return qset or self.none()
    ...
    def filter_for_dd_notifycategory_field(
        self, request, choices=False, sitewise=False
    ):
        S = request.session
        qset = self.filter(
            Q(
        ...
        if choices:
            qset = qset.annotate(
                text=Concat(F("taname"), V(" ("), F("tacode"), V(")"))
            ).values_list("id", "text")
        return qset or self.none()
    ...
    def get_choices_for_worktype(self, request):
        S = request.session
        qset = (
            self.annotate(custom_field=V("", output_field=models.CharField()))
            .filter(tatype__tacode="WORKTYPE", client_id=S["client_id"], enable=True)
    ...
    def get_asset_types_choices(self, request):
        ...
        qset = (
            self.select_related("tatype")
            .filter(client_id=S["client_id"], tatype__tacode="ASSETTYPE")
            .values_list("id", "tacode")
            .distinct()
        )
        # add an extra choice with empty strings
        qset = [("", "")] + list(qset)
...
class GeofenceManager(models.Manager):
    ...
    def get_gfs_for_siteids(self, siteids):
        ...
        if (
            qset := self.annotate(geofencecoords=AsGeoJSON("geofence"))
            .select_related(*self.related)
            .filter(bu_id__in=siteids)
            .values(*self.fields)
        ):
            geofencestring = ""
            for obj in qset:
    ...
    def getPeoplesGeofence(self, request):
        ...
        qset = (
            qset.annotate(text=Concat("peoplename", V(" ("), "peoplecode", V(")")))
            .values("text", "id")
            .distinct()
        )
        return qset or self.none()
...
class ShiftManager(models.Manager):
    ...
    def shift_listview(self, request, related, fields):
        ...
        return (
            ...
            .annotate(
                dsgn=Concat(
                    F("designation__taname"), V(" ("), F("designation__tacode"), V(")")
                )
            )
            .select_related("designation", "bu")
            .values(
                "id",
                "shiftname",
                "dsgn",
    ...
    def get_designations_data(self, request, id):
        S = request.session
        return (
            self.filter(client_id=S["client_id"], bu_id=S["bu_id"], id=id).values(
                "designation__taname", "designation__tacode", "peoplecount"
            )
            or self.none()
        )
    ...
    def get_shiftcontract_details(self, request, id):
        ...
        qset = self.filter(
            bu_id=S["bu_id"], client_id=S["client_id"], enable=True
        ).values("id", "peoplecount", "shift_data")
    ...
    def get_shift_data(self, buid, clientid, mdtz):
        data = self.filter(
            ...
            "id", "shiftname", "cdtz", "mdtz", "starttime", "endtime", "shift_data"
        )
...
class SubscriptionManger(models.Manager):
    ...
    def handle_subscription_postdata(self, request):
        ...
        if R["action"] == "create":
            if self.filter(
                startdate=R["startdate"], enddate=R["enddate"], client_id=R["client_id"]
            ).exists():
                return {
                    "data": list(self.none()),
    ...
    def get_license_list(self, client_id):
        qset = self.filter(
            client_id=client_id, status=self.model.StatusChoices.A.value
        ).values(
            "id",
            "startdate",
            "enddate",
    ...
    def get_terminated_license_list(self, client_id):
        qset = self.filter(
            status=self.model.StatusChoices.IA.value, client_id=client_id
        ).values("id", "terminateddate", "enddate", "reason", "assignedhandset")
        return qset or self.none()
