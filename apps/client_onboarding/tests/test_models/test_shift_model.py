...
class TestShiftModel:
    ...
    def test_shift_relationships(
        self, shift_factory, test_client_bt, test_bu_bt, test_designation_type
    ):
        """Test Shift foreign key relationships"""
        shift = shift_factory(
            client=test_client_bt, bu=test_bu_bt, designation=test_designation_type
        )
    ...
    def test_shift_unique_constraint(
        self, shift_factory, test_client_bt, test_bu_bt, test_designation_type
    ):
    ...
    def test_shift_people_count_field(self, shift_factory):
        ...
        shift = shift_factory(peoplecount=25)

        assert shift.peoplecount == 25

        # Test with None (should be allowed)
        shift_no_count = shift_factory(peoplecount=None)
    ...
    def test_shift_night_shift_flag(self, shift_factory):
        ...
        night_shift = shift_factory(shiftname="Night Shift", nightshiftappicable=True)

        assert night_shift.nightshiftappicable is True

        # Test night shift disabled
        day_shift = shift_factory(shiftname="Day Shift", nightshiftappicable=False)
    ...
    def test_shift_captcha_frequency(self, shift_factory):
        ...
        shift = shift_factory(captchafreq=30)
    ...
    def test_shift_data_json_field(self, shift_factory, test_designation_type):
        ...
        shift = shift_factory(shift_data=shift_data, designation=test_designation_type)
    ...
    def test_shift_enable_disable_functionality(self, shift_factory):
        ...
        enabled_shift = shift_factory(shiftname="Enabled Shift", enable=True)

        disabled_shift = shift_factory(shiftname="Disabled Shift", enable=False)
    ...
    def test_shift_tenant_aware_functionality(
        self, shift_factory, test_client_bt, test_bu_bt
    ):
        """Test Shift tenant-aware model functionality"""
        shift = shift_factory(
            shiftname="Tenant Test Shift", client=test_client_bt, bu=test_bu_bt
        )
    ...
    def test_shift_bulk_operations(self, shift_factory, test_client_bt, test_bu_bt):
        ...
        bulk_enabled = Shift.objects.filter(
            shiftname__startswith="Bulk Shift", enable=True
        )
        bulk_disabled = Shift.objects.filter(
            shiftname__startswith="Bulk Shift", enable=False
        )
        ...
        # Test bulk update
        Shift.objects.filter(shiftname__startswith="Bulk Shift").update(captchafreq=20)

        # Verify bulk update
        updated_shifts = Shift.objects.filter(
            shiftname__startswith="Bulk Shift", captchafreq=20
        )
        assert updated_shifts.count() == 15

    def test_shift_filtering_by_client_and_bu(
        self, shift_factory, test_client_bt, test_bu_bt
    ):
        ...
        other_client = test_client_bt.__class__.objects.create(
            bucode="OTHER_CLIENT", buname="Other Client", enable=True
        )

        other_bu = test_bu_bt.__class__.objects.create(
            bucode="OTHER_BU", buname="Other BU", enable=True
        )

        # Create shifts for different clients and BUs
        shift1 = shift_factory(
            shiftname="Client1 BU1 Shift", client=test_client_bt, bu=test_bu_bt
        )

        shift2 = shift_factory(
            shiftname="Client1 BU2 Shift", client=test_client_bt, bu=other_bu
        )

        shift3 = shift_factory(
            shiftname="Client2 BU1 Shift", client=other_client, bu=test_bu_bt
        )

        shift4 = shift_factory(
            shiftname="Client2 BU2 Shift", client=other_client, bu=other_bu
        )
    ...
    def test_shift_designation_specific_data(self, shift_factory, typeassist_factory):
        ...
        supervisor_designation = typeassist_factory(
            tacode="SUPERVISOR", taname="Supervisor"
        )
    ...
    def test_shift_performance_queries(self, shift_factory, test_client_bt, test_bu_bt):
        ...
        enabled_count = Shift.objects.filter(
            shiftname__startswith="Performance Shift", enable=True
        ).count()
        disabled_count = Shift.objects.filter(
            shiftname__startswith="Performance Shift", enable=False
        ).count()
        ...
        night_shifts = Shift.objects.filter(
            shiftname__startswith="Performance Shift", nightshiftappicable=True
        )
        day_shifts = Shift.objects.filter(
            shiftname__startswith="Performance Shift", nightshiftappicable=False
        )
