...
def notify_wo_creation(id):
    ...
    if wo := Wom.objects.filter(id=id).first():
        ...
        if atts := check_attachments_if_any(wo):
            attachments = [
                f"{settings.MEDIA_ROOT}/{att['filepath']}{att['filename']}"
                for att in atts
            ]

        html_message = render_to_string(
            "work_order_management/work_order_email.html", context=context
        )
        send_email(
            subject=subject,
            body=html_message,
            to=emails,
            atts=attachments if atts else None,
        )
        wo.ismailsent = True
        wo.save()
        return wo
...
def check_all_verified(womuuid, usercode):
    ...
    for verifier in w.other_data["wp_verifiers"]:
        ...
        if verifier["peoplecode"] == usercode:
            ...
            logger.info(
                f"verifier {verifier['name']} has approved with status code {verifier['status']}"
            )
...
def check_all_approved(womuuid, usercode):
    ...
    for approver in w.other_data["wp_approvers"]:
        ...
        if approver["peoplecode"] == usercode:
            ...
            logger.info(
                f"approver {approver['name']} has approved with status code {approver['status']}"
            )
...
def save_approvers_injson(wp):
    ...
    wp_approvers = [
        {
            "name": People.objects.get(peoplecode=approver).peoplename,
            "status": "PENDING",
            "identifier": "APPROVER",
            "peoplecode": approver,
        }
        for approver in wp.approvers
    ]
...
def save_verifiers_injson(wp):
    ...
    wp_verifiers = [
        {
            "name": People.objects.get(peoplecode=verifier).peoplename,
            "status": "PENDING",
            "identifier": "VERIFIER",
            "peoplecode": verifier,
        }
        for verifier in wp.verifiers
    ]
...
def handle_valid_form(form, R, request, create):
    ...
    sla = putils.save_userinfo(sla, request.user, request.session, create=create)
...
def create_sla_details(R, wom, request, formdata):
    ...
    for k, v in formdata.items():
        if (
            ...
            if (
                qsb_obj.answertype in ["NUMERIC"]
                and qsb_obj.alerton
                and len(qsb_obj.alerton) > 0
            ):
...
def get_overall_score(id):
    (
        ...
        all_average_score,
        remarks,
    ) = Wom.objects.sla_data_for_report(id)
    ...
    for i in range(1, 13):
        ...
        if latest_report["max_date"]:
            ...
            sla_reports[month_year] = [
                report.other_data["overall_score"],
                report.other_data.get("uptime_score", "N/A"),
            ]
        else:
...
def approvers_email_and_name(people_codes):
    ...
    for people_code in people_codes:
        data = People.objects.filter(peoplecode=people_code).values(
            "peoplename", "email"
        )[0]
