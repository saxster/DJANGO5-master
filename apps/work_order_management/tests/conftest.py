...
def test_client_wom():
    ...
    return Bt.objects.create(
        bucode="WOMCLIENT", buname="Work Order Management Client", enable=True
    )
...
def test_vendor_type():
    ...
    return TypeAssist.objects.create(
        tacode="ELECTRICAL", taname="Electrical Contractor", enable=True
    )
...
def test_ticket_category():
    ...
    return TypeAssist.objects.create(
        tacode="MAINTENANCE", taname="Maintenance Request", enable=True
    )
...
def test_question():
    ...
    return Question.objects.create(quesname="What is the issue?", enable=True)
    ...
    def _create_wom(
        ...
        **kwargs,
        ...
        defaults = {
            ...
            "plandatetime": kwargs.get(
                "plandatetime", timezone.now() + timedelta(hours=1)
            ),
            "expirydatetime": kwargs.get(
                "expirydatetime", timezone.now() + timedelta(hours=8)
            ),
...
def vendor_factory(test_client_wom, test_bu_wom, test_vendor_type):
    ...
    def _create_vendor(
        code=None, name=None, vendor_type=None, client=None, bu=None, **kwargs
    ):
        counter[0] += 1
        unique_id = f"{timezone.now().timestamp():.6f}_{counter[0]}"
        ...
        defaults = {
            ...
            "description": kwargs.get(
                "description", f"Test vendor description {counter[0]}"
            ),
        }
        defaults.update(kwargs)
...
def womdetails_factory(test_question):
    ...
    def _create_womdetails(wom=None, question=None, qset=None, **kwargs):
        counter[0] += 1
        ...
        if question is None:
            ...
            question = Question.objects.create(
                quesname=f"Question {unique_id}", enable=True
            )
...
def approver_factory(test_client_wom, test_bu_wom):
    ...
    def _create_approver(people=None, client=None, bu=None, identifier=None, **kwargs):
...
def priority_levels():
    ...
    return [Wom.Priority.HIGH, Wom.Priority.LOW, Wom.Priority.MEDIUM]
...
def vendor_types():
    ...
    for code, name in vendor_type_names:
        vendor_type = TypeAssist.objects.create(tacode=code, taname=name, enable=True)
