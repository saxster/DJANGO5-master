...
class TestApproverModel:
    ...
    def test_approver_identifier_choices(self, approver_factory):
        ...
        approver_role = approver_factory(
            identifier=Approver.Identifier.APPROVER, approverfor=["WORKORDER"]
        )

        # Test VERIFIER identifier
        verifier_role = approver_factory(
            identifier=Approver.Identifier.VERIFIER, approverfor=["WORKPERMIT"]
        )

        assert approver_role.identifier == Approver.Identifier.APPROVER
        assert verifier_role.identifier == Approver.Identifier.VERIFIER

    def test_approver_relationships(
        self, approver_factory, test_people_wom, test_client_wom, test_bu_wom
    ):
        """Test Approver foreign key relationships"""
        approver = approver_factory(
            people=test_people_wom, client=test_client_wom, bu=test_bu_wom
        )
    ...
    def test_approver_unique_constraint(self, approver_factory, test_people_wom):
        ...
        approver1 = approver_factory(
            people=test_people_wom, approverfor=approverfor_list, sites=sites_list
        )
    ...
    def test_approver_for_array_field(self, approver_factory):
        ...
        single_approver = approver_factory(approverfor=["WORKORDER"])
    ...
    def test_approver_sites_array_field(self, approver_factory):
        ...
        site_specific = approver_factory(
            sites=["SITE001", "SITE002"], forallsites=False
        )

        # All sites approver
        all_sites = approver_factory(sites=[], forallsites=True)
    ...
    def test_approver_for_all_sites_flag(self, approver_factory):
        ...
        # Global approver
        global_approver = approver_factory(forallsites=True, sites=[])

        # Site-specific approver
        site_approver = approver_factory(
            forallsites=False, sites=["SITE001", "SITE002", "SITE003"]
        )
    ...
    def test_approver_site_specific_permissions(
        self, approver_factory, test_client_wom, test_bu_wom
    ):
        """Test Approver site-specific permissions"""
        # Create additional sites
        site1 = test_bu_wom.__class__.objects.create(
            bucode="SITE001", buname="Site 1", parent=test_client_wom, enable=True
        )

        site2 = test_bu_wom.__class__.objects.create(
            bucode="SITE002", buname="Site 2", parent=test_client_wom, enable=True
        )
    ...
    def test_approver_bulk_operations(
        self, approver_factory, test_client_wom, test_bu_wom
    ):
        ...
        for i in range(15):
            approver = approver_factory(
                ...
                identifier=Approver.Identifier.APPROVER
                if i % 3 == 0
                else Approver.Identifier.VERIFIER,
        ...
        global_approvers = Approver.objects.filter(forallsites=True)
        assert global_approvers.count() == 4  # 0,4,8,12

    def test_approver_filtering_by_client_and_bu(
        self, approver_factory, test_client_wom, test_bu_wom
    ):
        ...
        other_client = test_client_wom.__class__.objects.create(
            bucode="OTHER_APP_CLIENT", buname="Other Approver Client", enable=True
        )

        other_bu = test_bu_wom.__class__.objects.create(
            bucode="OTHER_APP_BU", buname="Other Approver BU", enable=True
        )

        # Create approvers for different clients and BUs
        approver1 = approver_factory(
            approverfor=["WORKORDER"], client=test_client_wom, bu=test_bu_wom
        )

        approver2 = approver_factory(
            approverfor=["WORKPERMIT"], client=test_client_wom, bu=other_bu
        )

        approver3 = approver_factory(
            approverfor=["SLA_TEMPLATE"], client=other_client, bu=test_bu_wom
        )

        approver4 = approver_factory(
            approverfor=["WORKORDER"], client=other_client, bu=other_bu
        )
    ...
    def test_approver_hierarchy_permissions(
        self, approver_factory, test_people_wom, test_client_wom, test_bu_wom
    ):
    ...
    def test_approver_permission_combinations(self, approver_factory):
        ...
        wo_only = approver_factory(
            approverfor=["WORKORDER"], identifier=Approver.Identifier.APPROVER
        )

        # Work permit only verifier
        wp_only = approver_factory(
            approverfor=["WORKPERMIT"], identifier=Approver.Identifier.VERIFIER
        )

        # SLA template only approver
        sla_only = approver_factory(
            approverfor=["SLA_TEMPLATE"], identifier=Approver.Identifier.APPROVER
        )
    ...
    def test_approver_performance_queries(
        self, approver_factory, test_client_wom, test_bu_wom
    ):
        ...
        # Test count queries
        total_count = Approver.objects.filter(client=test_client_wom).count()
        assert total_count >= 60

        # Test responsibility-based filtering
        workorder_count = Approver.objects.filter(
            client=test_client_wom, approverfor__contains=["WORKORDER"]
        ).count()
        workpermit_count = Approver.objects.filter(
            client=test_client_wom, approverfor__contains=["WORKPERMIT"]
        ).count()
        sla_count = Approver.objects.filter(
            client=test_client_wom, approverfor__contains=["SLA_TEMPLATE"]
        ).count()

        assert (
            workorder_count == 20
        )  # Positions 0,3,6,9,12,15,18,21,24,27,30,33,36,39,42,45,48,51,54,57
        assert (
            workpermit_count == 20
        )  # Positions 1,4,7,10,13,16,19,22,25,28,31,34,37,40,43,46,49,52,55,58
        assert (
            sla_count == 20
        )  # Positions 2,5,8,11,14,17,20,23,26,29,32,35,38,41,44,47,50,53,56,59

        # Test identifier filtering
        approver_role_count = Approver.objects.filter(
            client=test_client_wom, identifier=Approver.Identifier.APPROVER
        ).count()
        verifier_role_count = Approver.objects.filter(
            client=test_client_wom, identifier=Approver.Identifier.VERIFIER
        ).count()
        ...
        global_approver_count = Approver.objects.filter(
            client=test_client_wom, forallsites=True
        ).count()

        assert (
            global_approver_count == 12
        )  # Every 5th record (0,5,10,15,20,25,30,35,40,45,50,55)
