...
class TestVendorModel:
    ...
    def test_vendor_str_representation(self, vendor_factory, test_vendor_type):
        ...
        vendor = vendor_factory(
            name="ABC Electrical Services", code="ELEC001", vendor_type=test_vendor_type
        )
    ...
    def test_vendor_str_representation_without_type(self, vendor_factory):
        ...
        vendor = vendor_factory(name="XYZ Services", code="XYZ001", vendor_type=None)
    ...
    def test_vendor_gps_location_field(self, vendor_factory, gps_coordinates_wom):
        ...
        vendor = vendor_factory(gpslocation=gps_coordinates_wom["vendor_location"])
    ...
    def test_vendor_relationships(
        self, vendor_factory, test_client_wom, test_bu_wom, test_vendor_type
    ):
        """Test Vendor foreign key relationships"""
        vendor = vendor_factory(
            client=test_client_wom, bu=test_bu_wom, vendor_type=test_vendor_type
        )
    ...
    def test_vendor_unique_constraint(self, vendor_factory, test_client_wom):
        ...
        # Create first vendor
        vendor1 = vendor_factory(code="UNIQUE001", client=test_client_wom)

        # Try to create duplicate - should raise IntegrityError
        with pytest.raises(IntegrityError):
            vendor_factory(code="UNIQUE001", client=test_client_wom)
    ...
    def test_vendor_enable_disable_functionality(self, vendor_factory):
        ...
        enabled_vendor = vendor_factory(
            code="ENABLED001", name="Enabled Vendor", enable=True
        )

        disabled_vendor = vendor_factory(
            code="DISABLED001", name="Disabled Vendor", enable=False
        )
    ...
    def test_vendor_show_to_all_sites_flag(
        self, vendor_factory, test_client_wom, test_bu_wom
    ):
    ...
    def test_vendor_description_field(self, vendor_factory):
        ...
        vendor = vendor_factory(code="DESC_VEN_001", description=description)

        assert vendor.description == description

        # Test with None description
        vendor_no_desc = vendor_factory(code="DESC_VEN_002", description=None)
    ...
    def test_vendor_bulk_operations(self, vendor_factory, test_client_wom, test_bu_wom):
        ...
        # Test bulk filtering
        bulk_enabled = Vendor.objects.filter(code__startswith="BULK_", enable=True)
        bulk_disabled = Vendor.objects.filter(code__startswith="BULK_", enable=False)
        ...
        global_vendors = Vendor.objects.filter(
            code__startswith="BULK_", show_to_all_sites=True
        )
        assert global_vendors.count() == 7  # Every 3rd record (0,3,6,9,12,15,18)

        # Test bulk update
        Vendor.objects.filter(code__startswith="BULK_").update(email="bulk@vendor.com")

        # Verify bulk update
        updated_vendors = Vendor.objects.filter(
            code__startswith="BULK_", email="bulk@vendor.com"
        )
        assert updated_vendors.count() == 20

    def test_vendor_filtering_by_client_and_bu(
        self, vendor_factory, test_client_wom, test_bu_wom
    ):
        ...
        other_client = test_client_wom.__class__.objects.create(
            bucode="OTHER_VEN_CLIENT", buname="Other Vendor Client", enable=True
        )

        other_bu = test_bu_wom.__class__.objects.create(
            bucode="OTHER_VEN_BU", buname="Other Vendor BU", enable=True
        )

        # Create vendors for different clients and BUs
        vendor1 = vendor_factory(
            code="V1", name="Client1 BU1 Vendor", client=test_client_wom, bu=test_bu_wom
        )

        vendor2 = vendor_factory(
            code="V2", name="Client1 BU2 Vendor", client=test_client_wom, bu=other_bu
        )

        vendor3 = vendor_factory(
            code="V3", name="Client2 BU1 Vendor", client=other_client, bu=test_bu_wom
        )

        vendor4 = vendor_factory(
            code="V4", name="Client2 BU2 Vendor", client=other_client, bu=other_bu
        )
    ...
    def test_vendor_gps_location_queries(self, vendor_factory, gps_coordinates_wom):
        ...
        vendors_with_location = Vendor.objects.filter(gpslocation__isnull=False)
        vendors_without_location = Vendor.objects.filter(gpslocation__isnull=True)
    ...
    def test_vendor_contact_validation_scenarios(self, vendor_factory):
        ...
        valid_vendor = vendor_factory(mobno="9876543210", email="valid@vendor.com")

        assert valid_vendor.mobno == "9876543210"
        assert valid_vendor.email == "valid@vendor.com"

        # Test with different mobile number formats
        mobile_formats = ["9876543210", "+919876543210", "91-9876543210"]

        for i, mobile in enumerate(mobile_formats):
            vendor = vendor_factory(code=f"MOB_{i}", mobno=mobile)
    ...
    def test_vendor_tenant_aware_functionality(
        self, vendor_factory, test_client_wom, test_bu_wom
    ):
    ...
    def test_vendor_performance_queries(
        self, vendor_factory, test_client_wom, test_bu_wom
    ):
        ...
        # Test count queries
        total_count = Vendor.objects.filter(code__startswith="PERF_").count()
        assert total_count == 100

        # Test enabled/disabled filtering
        enabled_count = Vendor.objects.filter(
            code__startswith="PERF_", enable=True
        ).count()
        disabled_count = Vendor.objects.filter(
            code__startswith="PERF_", enable=False
        ).count()
        ...
        global_vendor_count = Vendor.objects.filter(
            code__startswith="PERF_", show_to_all_sites=True
        ).count()

        assert global_vendor_count == 20  # Every 5th record

    def test_vendor_address_and_location_combined(
        self, vendor_factory, gps_coordinates_wom
    ):
    ...
    def test_vendor_multi_site_scenarios(self, vendor_factory, test_client_wom):
        ...
        bu1 = test_client_wom.__class__.objects.create(
            bucode="SITE1", buname="Site 1", parent=test_client_wom, enable=True
        )

        bu2 = test_client_wom.__class__.objects.create(
            bucode="SITE2", buname="Site 2", parent=test_client_wom, enable=True
        )

        bu3 = test_client_wom.__class__.objects.create(
            bucode="SITE3", buname="Site 3", parent=test_client_wom, enable=True
        )
        ...
        site1_vendors = Vendor.objects.filter(client=test_client_wom, bu=bu1)

        all_site_vendors = Vendor.objects.filter(
            client=test_client_wom, show_to_all_sites=True
        )
