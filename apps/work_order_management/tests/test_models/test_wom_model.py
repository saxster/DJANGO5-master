...
class TestWomModel:
    ...
    def test_wom_work_status_choices(self, wom_factory, work_order_statuses):
        ...
        for status in work_order_statuses:
            wom = wom_factory(
                workstatus=status, description=f"Work Order with {status} status"
            )
    ...
    def test_wom_priority_choices(self, wom_factory, priority_levels):
        ...
        for priority in priority_levels:
            wom = wom_factory(
                priority=priority, description=f"Work Order with {priority} priority"
            )
            assert wom.priority == priority

    def test_wom_identifier_choices(self, wom_factory):
        """Test Wom identifier choices"""
        identifiers = [Wom.Identifier.WO, Wom.Identifier.WP, Wom.Identifier.SLA]

        for identifier in identifiers:
            wom = wom_factory(
                identifier=identifier, description=f"Item with {identifier} identifier"
            )
    ...
    def test_wom_array_fields(self, wom_factory):
        ...
        wom = wom_factory(
            approvers=approvers, verifiers=verifiers, categories=categories
        )
    ...
    def test_wom_json_fields(self, wom_factory, sample_other_data, sample_wo_history):
        ...
        wom = wom_factory(other_data=sample_other_data, wo_history=sample_wo_history)
    ...
    def test_wom_geojson_field(self, wom_factory):
        ...
        wom = wom_factory(geojson=geojson_data)
    ...
    def test_wom_hierarchy_parent_child(self, wom_factory):
        ...
        parent_wom = wom_factory(
            description="Parent Work Order", identifier=Wom.Identifier.WP
        )
    ...
    def test_wom_boolean_flags(self, wom_factory):
        ...
        wom = wom_factory(alerts=True, ismailsent=True, isdenied=True)
    ...
    def test_wom_attachment_count(self, wom_factory):
        ...
        wom = wom_factory(attachmentcount=5)
    ...
    def test_wom_unique_constraint(
        self, wom_factory, test_questionset, test_client_wom
    ):
        """Test Wom unique constraint on qset, client, id"""
        # Create first work order
        wom1 = wom_factory(qset=test_questionset, client=test_client_wom)
        ...
        wom2 = wom_factory(qset=test_questionset, client=test_client_wom)
    ...
    def test_wom_add_history_method(self, wom_factory):
        ...
        wom = wom_factory(description="Test work order for history")
    ...
    def test_wom_bulk_operations(self, wom_factory, test_client_wom, test_bu_wom):
        ...
        # Test bulk update
        Wom.objects.filter(description__startswith="Bulk Work Order").update(
            ismailsent=True
        )

        # Verify bulk update
        updated_woms = Wom.objects.filter(
            description__startswith="Bulk Work Order", ismailsent=True
        )
        assert updated_woms.count() == 15

    def test_wom_filtering_by_client_and_bu(
        self, wom_factory, test_client_wom, test_bu_wom
    ):
        ...
        other_client = test_client_wom.__class__.objects.create(
            bucode="OTHER_WOM_CLIENT", buname="Other WOM Client", enable=True
        )

        other_bu = test_bu_wom.__class__.objects.create(
            bucode="OTHER_WOM_BU", buname="Other WOM BU", enable=True
        )

        # Create work orders for different clients and BUs
        wom1 = wom_factory(
            description="Client1 BU1 Work Order", client=test_client_wom, bu=test_bu_wom
        )

        wom2 = wom_factory(
            description="Client1 BU2 Work Order", client=test_client_wom, bu=other_bu
        )

        wom3 = wom_factory(
            description="Client2 BU1 Work Order", client=other_client, bu=test_bu_wom
        )

        wom4 = wom_factory(
            description="Client2 BU2 Work Order", client=other_client, bu=other_bu
        )
    ...
    def test_wom_performance_queries(self, wom_factory, test_client_wom, test_bu_wom):
        ...
        for i in range(100):
            wom = wom_factory(
                ...
                workpermit=Wom.WorkPermitStatus.NOTNEED
                if i % 3 == 0
                else Wom.WorkPermitStatus.PENDING,
                client=test_client_wom,
                bu=test_bu_wom,
            )
            woms.append(wom)
        ...
        high_priority_count = Wom.objects.filter(
            description__startswith="Performance Work Order", priority=Wom.Priority.HIGH
        ).count()

        assert high_priority_count == 20  # Every 5th record

    def test_wom_tenant_aware_functionality(
        self, wom_factory, test_client_wom, test_bu_wom
    ):
        """Test Wom tenant-aware model functionality"""
        wom = wom_factory(
            description="Tenant Test Work Order", client=test_client_wom, bu=test_bu_wom
        )
