...
class TestWomDetailsModel:
    ...
    def test_womdetails_answer_type_choices(self, womdetails_factory, answer_types):
        ...
        for answer_type in answer_types[:5]:  # Test first 5 to avoid too many objects
            womdetails = womdetails_factory(
                answertype=answer_type, answer=f"Test answer for {answer_type}"
            )
    ...
    def test_womdetails_relationships(
        self, womdetails_factory, wom_factory, test_question, test_questionset
    ):
        """Test WomDetails foreign key relationships"""
        wom = wom_factory()

        womdetails = womdetails_factory(
            wom=wom, question=test_question, qset=test_questionset
        )
    ...
    def test_womdetails_unique_constraint(
        self, womdetails_factory, wom_factory, test_question
    ):
        """Test WomDetails unique constraint on question, wom"""
        wom = wom_factory()

        # Create first WomDetails
        womdetails1 = womdetails_factory(question=test_question, wom=wom)

        # Try to create duplicate - should raise IntegrityError
        with pytest.raises(IntegrityError):
            womdetails_factory(question=test_question, wom=wom)
    ...
    def test_womdetails_date_time_answers(self, womdetails_factory):
        ...
        date_womdetails = womdetails_factory(
            answertype=WomDetails.AnswerType.DATE, answer="2024-01-15"
        )

        # Time answer
        time_womdetails = womdetails_factory(
            answertype=WomDetails.AnswerType.TIME, answer="14:30:00"
        )
    ...
    def test_womdetails_text_answer_types(self, womdetails_factory):
        ...
        singleline_womdetails = womdetails_factory(
            answertype=WomDetails.AnswerType.SINGLELINE, answer="Short text answer"
        )
        ...
        email_womdetails = womdetails_factory(
            answertype=WomDetails.AnswerType.EMAILID, answer="user@example.com"
        )
    ...
    def test_womdetails_sequence_ordering(self, womdetails_factory, wom_factory):
        ...
        womdetails_1 = womdetails_factory(wom=wom, seqno=1, answer="First question")

        womdetails_3 = womdetails_factory(wom=wom, seqno=3, answer="Third question")

        womdetails_2 = womdetails_factory(wom=wom, seqno=2, answer="Second question")
    ...
    def test_womdetails_bulk_operations(self, womdetails_factory, wom_factory):
        ...
        for i in range(10):
            womdetails = womdetails_factory(
                ...
                answertype=WomDetails.AnswerType.SINGLELINE
                if i % 2 == 0
                else WomDetails.AnswerType.NUMERIC,
                answer=f"Answer {i}",
        ...
        singleline_details = WomDetails.objects.filter(
            wom=wom, answertype=WomDetails.AnswerType.SINGLELINE
        )
        numeric_details = WomDetails.objects.filter(
            wom=wom, answertype=WomDetails.AnswerType.NUMERIC
        )
        ...
        mandatory_details = WomDetails.objects.filter(wom=wom, ismandatory=True)
        assert mandatory_details.count() == 4  # Positions 0, 3, 6, 9

        # Test bulk update
        WomDetails.objects.filter(wom=wom).update(alerts=True)

        # Verify bulk update
        updated_details = WomDetails.objects.filter(wom=wom, alerts=True)
    ...
    def test_womdetails_validation_scenarios(self, womdetails_factory):
        ...
        # Email validation
        email_womdetails = womdetails_factory(
            answertype=WomDetails.AnswerType.EMAILID, answer="test@example.com"
        )
    ...
    def test_womdetails_performance_queries(self, womdetails_factory, wom_factory):
        ...
        for answer_type in answer_types:
            type_count = WomDetails.objects.filter(
                wom=wom, answertype=answer_type
            ).count()
            assert type_count > 0  # Each type should have some records

        # Test mandatory field count
        mandatory_count = WomDetails.objects.filter(wom=wom, ismandatory=True).count()
        assert mandatory_count == 10  # Every 5th record

        # Test attachment count
        attachment_count = WomDetails.objects.filter(wom=wom, isavpt=True).count()
