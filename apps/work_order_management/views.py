...
class WorkPermit(LoginRequiredMixin, View):
    ...
    def get(self, request, *args, **kwargs):
        ...
        if action == "verifier_approve_wp" and R.get("womid"):
            ...
            try:
                result = work_order_service.verify_work_permit(
                    ...
                    user_data={"request": request},  # Pass request for PDF generation
                )
        ...
        if R.get("qsetid"):
            ...
            # Only remove the last section if it's specifically a return work permit section
            rwp_details = []
            if len(wp_details) > 1:
                last_section = wp_details[-1]
                # Check if the last section name suggests it's a return work permit section
