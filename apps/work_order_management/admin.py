...
class VendorResource(resources.ModelResource):
    ...
    SHOWTOALLSITES = fields.Field(
        attribute="show_to_all_sites",
        column_name="Applicable to All Sites",
        default=False,
    )
    ...
    def before_import_row(self, row, row_number=None, **kwargs):
        ...
        # mob no validation
        # if not verify_mobno(str(row.get('Mob No*', -1))): raise ValidationError("Mob No* is not valid")
        ...
        # unique record check
        if (
            wom.Vendor.objects.select_related()
            .filter(code=row["Code*"], client__bucode=row["Client*"])
            .exists()
        ):
            raise ValidationError(
