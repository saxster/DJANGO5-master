...
class TestGeneratePDFModel:
    ...
    def test_generate_pdf_document_type_choices(
        self, generate_pdf_factory, document_type_choices
    ):
    ...
    def test_generate_pdf_company_choices(self, generate_pdf_factory, company_choices):
        ...
        for company in company_choices:
            generate_pdf = generate_pdf_factory(
                company=company, customer=f"Customer for {company.value}"
            )
            assert generate_pdf.company == company

    def test_generate_pdf_additional_filter_choices(
        self, generate_pdf_factory, additional_filter_choices
    ):
    ...
    def test_generate_pdf_form_type_choices(
        self, generate_pdf_factory, form_type_choices
    ):
        """Test GeneratePDF form type choices"""
        for form_type in form_type_choices:
            generate_pdf = generate_pdf_factory(
                type_of_form=form_type, customer=f"Customer for {form_type.value}"
            )
    ...
    def test_generate_pdf_period_formats(self, generate_pdf_factory):
        ...
        for period in period_formats:
            generate_pdf = generate_pdf_factory(
                period_from=period, customer=f"Customer for period {period}"
            )
    ...
    def test_generate_pdf_performance_queries(self, generate_pdf_factory):
        ...
        for i in range(100):
            pdf = generate_pdf_factory(
                document_type=GeneratePDF.DocumentType.PAYROLL
                if i % 2 == 0
                else GeneratePDF.DocumentType.PF,
                company=GeneratePDF.Company.SPS
                if i % 3 == 0
                else GeneratePDF.Company.SFS,
        ...
        sps_count = GeneratePDF.objects.filter(company=GeneratePDF.Company.SPS).count()
        sfs_count = GeneratePDF.objects.filter(company=GeneratePDF.Company.SFS).count()
