...
class TestScheduleReportModel:
    ...
    def test_schedule_report_str_representation(self, schedule_report_factory):
        ...
        schedule_report = schedule_report_factory(
            report_name="Daily Task Summary", report_type="TASKSUMMARY"
        )
    ...
    def test_schedule_report_template_choices(
        self, schedule_report_factory, report_template_choices
    ):
        ...
        for template_code, template_name in report_template_choices[:5]:  # Test first 5
            if template_code:  # Skip empty choice
                schedule_report = schedule_report_factory(
                    report_type=template_code, report_name=f"{template_name} Schedule"
                )
                assert schedule_report.report_type == template_code

    def test_schedule_report_relationships(
        self, schedule_report_factory, test_client_reports, test_bu_reports
    ):
        """Test ScheduleReport foreign key relationships"""
        schedule_report = schedule_report_factory(
            client=test_client_reports, bu=test_bu_reports
        )
    ...
    def test_schedule_report_working_days_choices(
        self, schedule_report_factory, working_days_choices
    ):
        """Test ScheduleReport working days choices"""
        for days_code, days_name in working_days_choices:
            schedule_report = schedule_report_factory(
                workingdays=days_code, report_name=f"Report for {days_name}"
            )
            assert schedule_report.workingdays == days_code

    def test_schedule_report_cron_expressions(
        self, schedule_report_factory, sample_cron_expressions
    ):
    ...
    def test_schedule_report_email_arrays(
        self, schedule_report_factory, sample_email_data
    ):
    ...
    def test_schedule_report_json_params_field(
        self, schedule_report_factory, sample_report_params
    ):
        """Test ScheduleReport report_params JSON field"""
        schedule_report = schedule_report_factory(
            report_params={"report_params": sample_report_params}
        )

        assert schedule_report.report_params is not None
        assert (
            schedule_report.report_params["report_params"]["date_range"]
            == "last_30_days"
        )
    ...
    def test_schedule_report_time_field(self, schedule_report_factory):
        ...
        for send_time in send_times:
            schedule_report = schedule_report_factory(
                report_sendtime=send_time, report_name=f"Report at {send_time}"
            )
            assert schedule_report.report_sendtime == send_time

    def test_schedule_report_enable_disable_functionality(
        self, schedule_report_factory
    ):
        """Test ScheduleReport enable/disable functionality"""
        enabled_report = schedule_report_factory(
            report_name="Enabled Report", enable=True
        )

        disabled_report = schedule_report_factory(
            report_name="Disabled Report", enable=False
        )
    ...
    def test_schedule_report_unique_constraints(
        self, schedule_report_factory, test_client_reports, test_bu_reports
    ):
    ...
    def test_schedule_report_complex_params(self, schedule_report_factory):
        ...
        complex_params = {
            "report_params": {
                ...
                "grouping": {"primary": "asset_category", "secondary": "status"},
                "output": {
                    "format": "pdf",
                    "include_charts": True,
        ...
        schedule_report = schedule_report_factory(
            report_name="Complex Parameters Report", report_params=complex_params
        )
    ...
    def test_schedule_report_bulk_operations(
        self, schedule_report_factory, test_client_reports, test_bu_reports
    ):
    ...
    def test_schedule_report_filtering_by_client_and_bu(
        self, schedule_report_factory, test_client_reports, test_bu_reports
    ):
        ...
        other_client = test_client_reports.__class__.objects.create(
            bucode="OTHERSCHDCLIENT", buname="Other Schedule Client", enable=True
        )
    ...
    def test_schedule_report_cron_validation_scenarios(self, schedule_report_factory):
        ...
        for cron_expr, description in cron_scenarios:
            schedule_report = schedule_report_factory(
                cron=cron_expr, report_name=f"Report: {description}", crontype="CUSTOM"
            )
            assert schedule_report.cron == cron_expr
    ...
    def test_schedule_report_email_recipients_scenarios(self, schedule_report_factory):
        ...
        no_cc_recipients = schedule_report_factory(
            report_name="No CC Report", to_addr=["operations@company.com"], cc=[]
        )
    ...
    def test_schedule_report_performance_queries(
        self, schedule_report_factory, test_client_reports, test_bu_reports
    ):
        ...
        # Test count queries
        total_count = ScheduleReport.objects.filter(client=test_client_reports).count()
        assert total_count >= 100

        # Test report type filtering
        task_summary_count = ScheduleReport.objects.filter(
            client=test_client_reports, report_type="TASKSUMMARY"
        ).count()
        tour_summary_count = ScheduleReport.objects.filter(
            client=test_client_reports, report_type="TOURSUMMARY"
        ).count()
        ...
        enabled_count = ScheduleReport.objects.filter(
            client=test_client_reports, enable=True
        ).count()
