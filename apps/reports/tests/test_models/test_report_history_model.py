...
class TestReportHistoryModel:
    ...
    def test_report_history_str_representation(
        self, report_history_factory, test_user_reports
    ):
        """Test ReportHistory string representation"""
        report_history = report_history_factory(
            user=test_user_reports, report_name="Task Summary Report"
        )

        expected_str = (
            f"User: {test_user_reports.peoplename} Report: Task Summary Report"
        )
        assert str(report_history) == expected_str

    def test_report_history_export_type_choices(
        self, report_history_factory, export_type_choices
    ):
        """Test ReportHistory export type choices"""
        # Test each export type
        for export_type in export_type_choices:
            report_history = report_history_factory(
                export_type=export_type, report_name=f"Report {export_type.value}"
            )
            assert report_history.export_type == export_type

    def test_report_history_relationships(
        self,
        report_history_factory,
        test_user_reports,
        test_client_reports,
        test_bu_reports,
    ):
        ...
        report_history = report_history_factory(
            user=test_user_reports, client=test_client_reports, bu=test_bu_reports
        )
    ...
    def test_report_history_json_params_field(
        self, report_history_factory, sample_report_params
    ):
        """Test ReportHistory params JSON field"""
        report_history = report_history_factory(params=sample_report_params)
    ...
    def test_report_history_email_fields(
        self, report_history_factory, sample_email_data
    ):
    ...
    def test_report_history_has_data_flag(self, report_history_factory):
        ...
        report_with_data = report_history_factory(
            report_name="Assets Report", has_data=True
        )

        # Report without data
        report_without_data = report_history_factory(
            report_name="Empty Report", has_data=False
        )
    ...
    def test_report_history_traceback_field(self, report_history_factory):
        ...
        successful_report = report_history_factory(
            report_name="Successful Report", traceback=None
        )
    ...
    def test_report_history_timezone_handling(self, report_history_factory):
        ...
        for offset, tz_name in timezones:
            report_history = report_history_factory(
                report_name=f"Report for {tz_name}", ctzoffset=offset
            )
    ...
    def test_report_history_datetime_fields(self, report_history_factory):
        ...
        report_history = report_history_factory(datetime=custom_datetime)
    ...
    def test_report_history_filtering_by_user(
        self,
        report_history_factory,
        test_user_reports,
        test_client_reports,
        ...
        user1_report = report_history_factory(
            user=test_user_reports, report_name="User 1 Report"
        )

        user2_report = report_history_factory(
            user=other_user, report_name="User 2 Report"
        )
    ...
    def test_report_history_filtering_by_client_and_bu(
        self, report_history_factory, test_client_reports, test_bu_reports
    ):
        ...
        other_client = test_client_reports.__class__.objects.create(
            bucode="OTHERCLIENT", buname="Other Client", enable=True
        )

        other_bu = test_bu_reports.__class__.objects.create(
            bucode="OTHERBU", buname="Other BU", parent=other_client, enable=True
        )
        ...
        report2 = report_history_factory(
            client=test_client_reports, bu=other_bu, report_name="Client1 BU2 Report"
        )

        report3 = report_history_factory(
            client=other_client, bu=test_bu_reports, report_name="Client2 BU1 Report"
        )
    ...
    def test_report_history_bulk_operations(
        self, report_history_factory, test_user_reports
    ):
        ...
        download_reports = ReportHistory.objects.filter(
            export_type=ReportHistory.ExportType.D
        )
        email_reports = ReportHistory.objects.filter(
            export_type=ReportHistory.ExportType.E
        )
        scheduled_reports = ReportHistory.objects.filter(
            export_type=ReportHistory.ExportType.S
        )
    ...
    def test_report_history_complex_params(self, report_history_factory):
        ...
        complex_params = {
            "filters": {
                ...
                "sites": [1, 2, 3, 4],
                "assets": {"include": [101, 102, 103], "exclude": [999]},
            },
            "grouping": {
                "primary": "asset",
                "secondary": "status",
        ...
        report_history = report_history_factory(
            report_name="Complex Parameters Report", params=complex_params
        )
    ...
    def test_report_history_performance_queries(
        self,
        report_history_factory,
        test_user_reports,
        test_client_reports,
        ...
        for i in range(100):
            report = report_history_factory(
                ...
                export_type=ReportHistory.ExportType.D
                if i % 3 == 0
                else ReportHistory.ExportType.E,
        ...
        # Test count queries
        total_count = ReportHistory.objects.filter(client=test_client_reports).count()
        assert total_count >= 100

        # Test export type filtering
        download_count = ReportHistory.objects.filter(
            client=test_client_reports, export_type=ReportHistory.ExportType.D
        ).count()
        email_count = ReportHistory.objects.filter(
            client=test_client_reports, export_type=ReportHistory.ExportType.E
        ).count()
        ...
        reports_with_data_count = ReportHistory.objects.filter(
            client=test_client_reports, has_data=True
        ).count()

        assert reports_with_data_count == 25  # Every 4th record

    def test_report_history_date_range_queries(
        self, report_history_factory, report_time_ranges
    ):
        ...
        today_report = report_history_factory(
            report_name="Today Report", datetime=report_time_ranges["today"]["start"]
        )
