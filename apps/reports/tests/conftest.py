...
def test_client_reports():
    ...
    return Bt.objects.create(
        bucode="REPORTCLIENT", buname="Reports Test Client", enable=True
    )
...
def report_history_factory(test_user_reports, test_client_reports, test_bu_reports):
    ...
    def _create_report_history(
        user=None, export_type=None, report_name=None, client=None, bu=None, **kwargs
    ):
        counter[0] += 1
...
def schedule_report_factory(test_client_reports, test_bu_reports):
    ...
    def _create_schedule_report(
        report_type=None, report_name=None, client=None, bu=None, **kwargs
    ):
        counter[0] += 1
        unique_id = f"{timezone.now().timestamp():.6f}_{counter[0]}"
        ...
        defaults = {
            ...
            "fromdatetime": kwargs.get(
                "fromdatetime", timezone.now() - timedelta(days=1)
            ),
            "uptodatetime": kwargs.get(
                "uptodatetime", timezone.now() + timedelta(days=1)
            ),
            "lastgeneratedon": kwargs.get(
                "lastgeneratedon", timezone.now() - timedelta(hours=1)
            ),
            "report_params": kwargs.get(
                "report_params",
                {
                    "report_params": {
...
def generate_pdf_factory():
    ...
    def _create_generate_pdf(
        document_type=None, company=None, additional_filter=None, **kwargs
    ):
        counter[0] += 1
        unique_id = f"{timezone.now().timestamp():.6f}_{counter[0]}"
        ...
        defaults = {
            ...
            or GeneratePDF.AdditionalFilter.CUSTOMER,
...
@pytest.fixture
def authenticated_reports_request(
    rf, test_user_reports, test_client_reports, test_bu_reports
):
...
def sample_report_params():
    ...
    return {
        "date_range": "last_30_days",
        "filters": {"status": "active", "category": "maintenance", "priority": "high"},
        "output_format": "pdf",
        "include_charts": True,
...
def working_days_choices():
    ...
    return [("5", "Monday - Friday"), ("6", "Monday - Saturday")]
...
def additional_filter_choices():
    ...
    return [GeneratePDF.AdditionalFilter.CUSTOMER, GeneratePDF.AdditionalFilter.SITE]


@pytest.fixture
def form_type_choices():
    """All form type choices for GeneratePDF"""
    return [GeneratePDF.FormType.NORMALFORM, GeneratePDF.FormType.FORM16]
...
def report_time_ranges():
    ...
    return {
        ...
        "yesterday": {
            "start": (base_time - timedelta(days=1)).replace(
                hour=0, minute=0, second=0, microsecond=0
            ),
            "end": (base_time - timedelta(days=1)).replace(
                hour=23, minute=59, second=59, microsecond=999999
            ),
        },
        "last_week": {"start": base_time - timedelta(days=7), "end": base_time},
        "last_month": {"start": base_time - timedelta(days=30), "end": base_time},
        "last_quarter": {"start": base_time - timedelta(days=90), "end": base_time},
    }
