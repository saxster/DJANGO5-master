...
class BaseReportsExport(WeasyTemplateResponseMixin):
    ...
    def get_pdf_output(self):
        try:
            ...
            pdf_output = html.write_pdf(
                stylesheets=[css], font_config=font_config, presentational_hints=True
            )
            if self.returnfile:
                return pdf_output
    ...
    def get_excel_output(self, orm):
        ...
        output = self.excel_layout(
            workbook=workbook, worksheet=worksheet, df=df, writer=writer, output=output
        )
        return output
    ...
    def set_data_excel(self, orm=False):
        ...
        if orm:
            ...
            df.to_excel(
                writer, index=False, sheet_name="Sheet1", startrow=2, header=False
            )
    ...
    def create_attendance_report(self):
        ...
        sunday_style = workbook.add_format(
            {
                "font_size": 10,
                "align": "center",
                "valign": "vcenter",
        ...
        less_than_8_style = workbook.add_format(
            {
                "font_size": 10,
                "align": "center",
                "valign": "vcenter",
        ...
        less_than_4_style = workbook.add_format(
            {
                "font_size": 10,
                "align": "center",
                "valign": "vcenter",
        ...
        worksheet.merge_range(
            1, 0, 1, num_columns - 1, report_subtitle_site, subtitle_style
        )
        worksheet.merge_range(
            2, 0, 2, num_columns - 1, report_subtitle_date, subtitle_style
        )
        ...
        # Add headers
        headers = (
            ["Department", "Designation", "People Code", "People Name", "Values"]
            + header[0]
            + ["Total Hr's"]
        )
        ...
        for department, designations in data[0].items():
            ...
            for designation, people in designations.items():
                ...
                for person_code, records in people.items():
                    ...
                    worksheet.write(
                        current_row, 3, records[0]["peoplename"], cell_style
                    )
                ...
                if design_start_row < current_row - 1:
                    worksheet.merge_range(
                        design_start_row, 1, current_row - 1, 1, designation, cell_style
                    )

            # Merge department cells
            if dept_start_row < current_row - 1:
                worksheet.merge_range(
                    dept_start_row, 0, current_row - 1, 0, department, cell_style
                )
...
class ReportEssentials(object):
    ...
    def get_report_export_object(self):
        ...
        from apps.reports.report_designs.assetwise_task_status import (
            AssetwiseTaskStatus,
        )
        from apps.reports.report_designs.static_detailed_tour_summary import (
            StaticDetailedTourSummaryReport,
        )
        from apps.reports.report_designs.dynamic_tour_details import (
            DynamicTourDetailReport,
        )
        from apps.reports.report_designs.static_tour_details import (
            StaticTourDetailReport,
        )
        from apps.reports.report_designs.dynamic_detailed_tour_summary import (
            DynamicDetailedTourSummaryReport,
        )
        ...
        from apps.reports.report_designs.people_attendance_summary import (
            PeopleAttendanceSummaryReport,
        )
