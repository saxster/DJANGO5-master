...
from apps.work_order_management.utils import (
    get_last_12_months_sla_reports,
...
class ServiceLevelAgreement(BaseReportsExport):
    ...
    def set_context_data(self):
        ...
        (
            ...
            all_average_score,
            remarks,
        ) = Wom.objects.sla_data_for_report(self.formdata.get("id"))
        wom_details = Wom.objects.filter(id=self.formdata.get("id")).values_list(
            "vendor_id",
            "bu_id",
            "bu_id__buname",
            "other_data",
            "vendor_id__name",
            "vendor_id__description",
            "workpermit",
        )
        vendor_id = wom_details[0][0]
        site_id = wom_details[0][1]
        sitename = wom_details[0][2]
        sla_report_approvers = get_sla_report_approvers(
            wom_details[0][3]["wp_approvers"]
        )
        ...
        sla_last_three_month_report = get_last_12_months_sla_reports(
            vendor_id=vendor_id, bu_id=site_id, month_number=month_number
        )
