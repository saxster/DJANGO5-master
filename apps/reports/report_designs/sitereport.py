...
class SiteReportFormat(BaseReportsExport):
    ...
    def set_context_data(self):
        ...
        report_data = ReportQueryRepository.sitereport_report(
            timezone_str=self.args["timezone"],
            clientid=self.args["clientid"],
            sgroupids=self.args["sgroupids"],
            from_date=self.args["from"],
            upto_date=self.args["upto"]
        )
    ...
    def excel_columns(self, df):
        df = df[
            [
                ...
                "ATM MACHINE COUNT",
                "AC IN ATM COOLING",
                "ATM BACK ROOM LOCKED",
                "UPS ROOM BEHIND ATM LOBBY ALL SAFE",
                "BRANCH SHUTTER DAMAGED",
                "BRANCH PERIPHERY ROUND TAKEN",
                "AC ODU AND COPPER PIPE INTACT",
                "ANY WATER LOGGING OR FIRE IN VICINITY",
                "FE AVAILABLE IN ATM LOBBY",
                "DG DOOR LOCKED",
                "DAMAGE TO ATM LOBBY",
    ...
    def set_data(self):
        ...
        self.set_args_required_for_query()
        # Use new Django ORM implementation
        self.data = ReportQueryRepository.sitereport_report(
            timezone_str=self.args["timezone"],
            clientid=self.args["clientid"],
            sgroupids=self.args["sgroupids"],
            from_date=self.args["from"],
    ...
    def excel_layout(self, worksheet, workbook, df, writer, output):
        ...
        header_format = workbook.add_format(
            {"valign": "middle", "fg_color": "#01579b", "font_color": "white"}
        )
