...
class StaticTourDetailReport(BaseReportsExport):
    ...
    def get_data(self):
        ...
        children_prefetch = Prefetch(
            "jobneed_set", queryset=Jobneed.objects.select_related("asset")
        )
        ...
        for parent in parents:
            ...
            if dtimezone.is_aware(parent.plandatetime):
                plandatetime_naive = dtimezone.make_naive(
                    parent.plandatetime, dt_timezone.utc
                )
            else:
                plandatetime_naive = parent.plandatetime

            if dtimezone.is_aware(parent.expirydatetime):
                expirydatetime_naive = dtimezone.make_naive(
                    parent.expirydatetime, dt_timezone.utc
                )
            else:
            ...
            for child in checkpoints:  # Accessing the related set of child objects
                ...
                if child.starttime and dtimezone.is_aware(child.starttime):
                    starttime_naive = dtimezone.make_naive(
                        child.starttime, dt_timezone.utc
                    )
                else:
                    starttime_naive = child.starttime

                if child.endtime and dtimezone.is_aware(child.endtime):
                    endtime_naive = dtimezone.make_naive(child.endtime, dt_timezone.utc)
                else:
    ...
    def excel_layout(self, worksheet, workbook, df, writer, output):
        ...
        tour_cell_format = workbook.add_format(
            {"bold": True, "bg_color": "#eff7fc", "border": 1, "font_size": 10}
        )
        parent_row_format = workbook.add_format(
            {
                "bg_color": "#eff7fc",
                "border": 1,
                "font_size": 10,
        ...
        child_row_format = workbook.add_format(
            {"font_size": 10, "num_format": "yyyy-mm-dd hh:mm:ss"}
        )
        count_row_format = workbook.add_format(
            {
                "border": 1,
                "font_size": 10,
                "bold": True,
        ...
        for tour in self.data:
            ...
            worksheet.merge_range(
                f"B{row_num +1}:C{row_num +1}", tour["Tour/Route"], tour_cell_format
            )
            worksheet.write_row(
                row_num,
                3,
