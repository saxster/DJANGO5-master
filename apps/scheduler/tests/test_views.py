...
class RetriveSchdTasksJobTestCase(TestCase):
    ...
    def test_get_tasks_list(self):
        ...
        with patch.object(self.view.model.objects, "select_related") as mock_select:
            mock_queryset = Mock()
            # Mock the iterator() method to return an iterator
            mock_iterator = iter([])  # Empty iterator
            mock_queryset.filter.return_value.values.return_value.order_by.return_value.iterator.return_value = mock_iterator
            mock_select.return_value = mock_queryset
