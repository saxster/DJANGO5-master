...
# Suppress django-import-export field warnings for explicit whitelists
# These warnings are informational only and expected when using explicit fields lists
warnings.filterwarnings('ignore', message="ignoring field .* because not declared in 'fields' whitelist")

# Import validators - these are utility functions used throughout
try:
    from apps.core import utils
    clean_string = utils.clean_string
    validate_cron = utils.validate_cron
except (ImportError, AttributeError):
    # Fallback implementations if utils module refactored
    def clean_string(value, code=False):
        """Fallback clean_string implementation"""
        if not value:
            return value
        return str(value).strip()

    def validate_cron(cron_expr):
        """Fallback validate_cron implementation"""
        return True  # Basic validation - accept all
...
class TaskResource(BaseJobResource):
    ...
    class Meta:
        ...
        import_id_fields = ["ID"]
        report_skipped = True
        # NOTE: Using explicit fields list generates warnings for unmapped model fields
        # This is expected django-import-export behavior - warnings are informational only
        # Fields listed below are the ONLY fields exported/imported (security by design)
        fields = [
            "ID", "CLIENT", "SITE", "NAME", "DESC", "QSET", "ASSET",
            "PARENT", "PDURATION", "GRACETIME", "EXPTIME", "CRON", "FROMDATE",
            "UPTODATE", "SCANTYPE", "TKTCATEGORY", "PRIORITY", "PEOPLE",
            "PGROUP", "STARTTIME", "ENDTIME", "SEQNO", "IDF"
        ]
        # Cannot use 'exclude' with explicit 'fields' whitelist (mutually exclusive)
