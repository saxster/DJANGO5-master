...
from apps.core.utils_new.form_security import SecureFormMixin, FormValidators, InputSanitizer
...
class AssetForm(SecureFormMixin, forms.ModelForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.fields["parent"].queryset = Asset.objects.filter(
            ~Q(runningstatus="SCRAPPED"), identifier="ASSET", bu_id=S["bu_id"]
        )
        self.fields["location"].queryset = Location.objects.filter(
            ~Q(locstatus="SCRAPPED"), bu_id=S["bu_id"]
        )
        self.fields["type"].queryset = om.TypeAssist.objects.filter(
            Q(tatype__tacode__in=["ASSETTYPE", "ASSET_TYPE"]), client_id=S["client_id"]
        )
        self.fields["category"].queryset = om.TypeAssist.objects.filter(
            Q(tatype__tacode__in=["ASSETCATEGORY", "ASSET_CATEGORY"]),
            client_id=S["client_id"],
        )
        self.fields["subcategory"].queryset = om.TypeAssist.objects.filter(
            Q(tatype__tacode__in=["ASSETSUBCATEGORY", "ASSET_SUBCATEGORY"]),
            client_id=S["client_id"],
        )
        self.fields["unit"].queryset = om.TypeAssist.objects.filter(
            Q(tatype__tacode__in=["ASSETUNIT", "ASSET_UNIT", "UNIT"]),
            client_id=S["client_id"],
        )
        self.fields["brand"].queryset = om.TypeAssist.objects.filter(
            Q(tatype__tacode__in=["ASSETBRAND", "ASSET_BRAND", "BRAND"]),
            client_id=S["client_id"],
        )
        self.fields["servprov"].queryset = om.Bt.objects.filter(
            id=S["bu_id"], isserviceprovider=True, enable=True
        )
    ...
    def clean(self):
        ...
        self.cleaned_data[
            "gpslocation"
        ] = None  # Default value if gpslocation is missing
        if gpslocation:
        ...
        if self.cleaned_data.get("parent") is None:
            self.cleaned_data["parent"] = get_or_create_none_asset()
        ...
        if assetcode:
            ...
            if assetcode == getattr(
                self.cleaned_data.get("parent", None), "assetcode", None
            ):
            ...
            if (
                ...
                raise forms.ValidationError(
                    "Asset code already exists, choose a different code."
                )
    ...
    def clean_assetcode(self):
        val = self.cleaned_data.get("assetcode")
        if not val:
            return val

        # Sanitize the code using our security utilities
        val = InputSanitizer.sanitize_code(val)

        # Apply code validator
        FormValidators.code_validator(val)

        # Check for duplicates
        if (
            not self.instance.id
            and Asset.objects.filter(
                assetcode=val, client_id=self.request.session["client_id"]
            ).exists()
        ):
            raise forms.ValidationError("Asset with this code already exists")

        # Additional validation rules
        if val.endswith('.'):
            raise forms.ValidationError("Code cannot end with '.'")

        if len(val) < 2:
            raise forms.ValidationError("Code must be at least 2 characters long")

        return val

    def clean_assetname(self):
        """Validate and sanitize asset name."""
        val = self.cleaned_data.get("assetname")
        if not val:
            return val

        # Sanitize the name
        val = InputSanitizer.sanitize_name(val)

        # Apply validation
        FormValidators.validate_no_html(val)
        FormValidators.name_validator(val)
        ...
        if len(val) > 100:
            raise forms.ValidationError("Asset name cannot exceed 100 characters")

        return val.title()  # Title case for consistency
...
class AssetExtrasForm(forms.Form):
    ...
    is_nonengg_asset = forms.BooleanField(
        initial=False, required=False, label="Non Engg. Asset"
    )
    ...
    po_number = forms.CharField(
        max_length=55, label="Purchase Order Number", required=False
    )
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.fields["meter"].choices = (
            om.TypeAssist.objects.select_related("tatype")
            .filter(
                client_id=S["client_id"],
                tatype__tacode__in=["ASSETMETER", "ASSET_METER"],
            )
            .values_list("id", "tacode")
        )
        initailize_form_fields(self)
    ...
    def clean(self):
        ...
        if sfdate and stdate and sfdate > stdate:
            raise forms.ValidationError(
                "Service from date should be smaller than service to date!"
            )
...
class MasterAssetForm(forms.ModelForm):
    ...
    service = forms.ChoiceField(
        choices=SERVICE_CHOICES, initial="NONE", required=False, label="Service"
    )
    ...
    po_number = forms.CharField(
        required=False, label="Purchase Order Number", max_length=100
    )
...
class SmartPlaceForm(forms.ModelForm):
    error_msg = {"invalid_latlng": "Please enter a correct gps coordinates."}
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.fields["type"].queryset = om.TypeAssist.objects.filter(
            tatype__tacode__in=["ASSETTYPE", "ASSET_TYPE"]
        )
...
class CheckpointForm(forms.ModelForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.fields["location"].widget = s2forms.Select2Widget(
            attrs={"data-theme": "bootstrap5"}
        )
        self.fields["type"].widget = s2forms.Select2Widget(
            attrs={"data-theme": "bootstrap5"}
        )
        self.fields["assetcode"].widget.attrs = {"style": "text-transform:uppercase"}
        ...
        # filters for dropdown fields
        self.fields["location"].queryset = Location.objects.filter(
            Q(enable=True) | Q(loccode="NONE"), bu_id=S["bu_id"]
        )
        self.fields["parent"].queryset = Asset.objects.filter(
            Q(enable=True) | Q(assetcode="NONE"), bu_id=S["bu_id"]
        )
        self.fields["type"].queryset = om.TypeAssist.objects.filter(
            client_id=S["client_id"], tatype__tacode="CHECKPOINTTYPE"
        )
        initailize_form_fields(self)
    ...
    def clean(self):
        ...
        if " " in self.data.get("assetcode"):
            raise ValidationError(
                f": The '{self.data.get('assetcode')}' contains spaces, which is not allowed."
            )
...
class AssetComparisionForm(forms.Form):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.fields[
            "asset_type"
        ].choices = om.TypeAssist.objects.get_asset_types_choices(self.request)
        initailize_form_fields(self)
