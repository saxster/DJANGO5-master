...
class LocationForm(forms.ModelForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.fields["parent"].queryset = Location.objects.filter(
            client_id=S["client_id"], bu_id=S["bu_id"]
        )
        self.fields["type"].queryset = om.TypeAssist.objects.filter(
            client_id=S["client_id"], tatype__tacode="LOCATIONTYPE"
        )
    ...
    def clean(self):
        ...
        if loccode:
            ...
            if loccode == getattr(
                self.cleaned_data.get("parent", None), "loccode", None
            ):
            ...
            if (
                ...
                raise forms.ValidationError(
                    "Location code already exists, choose a different code."
                )
    ...
    def clean_loccode(self):
        if val := self.cleaned_data["loccode"]:
            if (
                not self.instance.id
                and Location.objects.filter(
                    loccode=val, client_id=self.request.session["client_id"]
                ).exists()
            ):
                raise forms.ValidationError(
                    "Location code already exist, choose different code"
                )
            if " " in val:
    ...
    def check_nones(self, cd):
        fields = {
            "parent": "get_or_create_none_location",
