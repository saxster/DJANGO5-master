...
class JobneedManager(models.Manager):
    ...
    def get_sitereportlist(self, request):
        ...
        from django.db.models import F, ExpressionWrapper, FloatField, DurationField, Count, OuterRef, Subquery, Value as V
        from django.db.models.functions import Cast, Concat, ExtractHour, ExtractMinute, ExtractSecond, Right,Round
        ...
        qset = self.filter(
            ...
            raw_time_spent=ExpressionWrapper(F('endtime') - F('starttime'), output_field=DurationField()),
            time_spent=Concat(
...
class JobneedDetailsManager(models.Manager):
    ...
    # def get_sitereport_que_ans(self, qset):
    #     que_answers = {}
    #     que_answers.clear()
    #     for i in qset:
    #         ques_ans = JobneedDetailsManager.objects.filter(jobneed_id = i['id']).select_related('question').values(
    #             'question__quesname', 'answertype', 'answer', 'min', 'max',
    #             'alerton', 'ismandatory', 'options', 'question_id','pk',
    #             'ctzoffset','seqno'
    #         ).order_by('seqno')
    #         que_answers[i['jobdesc']] = ques_ans
    #     return que_answers
    ...
    def get_task_details(self, taskid):
        ...
        qset = self.filter(
            ...
            'options', 'alerton', 'ismandatory', 'seqno','answer', 'alerts', 'uuid').order_by('seqno')
