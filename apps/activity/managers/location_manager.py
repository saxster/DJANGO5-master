...
class LocationManager(TenantAwareManager):
    ...
    def get_locations_modified_after(self, mdtz, buid, ctzoffset):
        ...
        qset = (
            ...
            .filter(Q(mdtz__gte=mdtz) & Q(bu_id__in=[buid]) & Q(enable=True))
            .values(*fields)
        )
        return qset or self.none()
    ...
    def filter_for_dd_location_field(self, request, choices=False, sitewise=False):
        ...
        if choices:
            qset = qset.annotate(
                text=Concat(F("locname"), V(" ("), F("loccode"), V(")"))
            ).values_list("id", "text")
        return qset or self.none()
    ...
    def location_type_choices_for_report(self, request):
        S = request.session
        qset = (
            self.filter(bu_id=S["bu_id"], client_id=S["client_id"])
            .select_related("type")
            .values_list("type_id", "type__taname")
            .distinct("type_id")
            .order_by("type_id")
        )
        return qset or self.none()
    ...
    def location_choices_for_report(self, request, choices=False, sitewise=False):
        S = request.session
        qset = self.filter(
            bu_id__in=S["assignedsites"], client_id=S["client_id"], enable=True
        )
        if sitewise:
            qset = qset.filter(bu_id=S["bu_id"])
        if choices:
