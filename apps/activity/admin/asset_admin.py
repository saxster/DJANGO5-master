...
class AssetResource(resources.ModelResource):
            ...
            raise ValidationError(
                {
                    ...
                    % {"identifier": asset_identifier, "valid": valid_idetifier_values}
                }
            )
            ...
            raise ValidationError(
                {
                    ...
                    % {"status": running_status, "valid": valid_running_status}
                }
            )
    ...
    def skip_row(self, instance, original, row, import_validation_errors=None):
        ...
        return row.get('_duplicate_found', False)
    ...
    def validations(self, row):
        ...
        logger.debug(
            "ASSETMETER: %s",
            (row.get("ASSETMETER", None), row.get("ASSET_METER", None)),
        )
        logger.debug("Client %s", row["Client*"])
        logger.debug("Service %s", row.get("Service", "N/A"))
        # unique record check - handle duplicates by raising a specific ValidationError
        existing_asset = (
        ...
        if existing_asset:
            # Log the duplicate but don't raise ValidationError - skip processing instead
            logger.warning(
                f"Duplicate record found - skipping: Code={row['Code*']}, Site={row['Site*']}, Client={row['Client*']}"
            )
            # Rather than raising error, we'll let skip_row handle this
            row['_duplicate_found'] = True

        if row.get("Service"):
            if row.get("Service") == "NONE":
                obj = get_or_create_none_typeassist()
                row["Service"] = obj.id
...
class AssetResourceUpdate(resources.ModelResource):
    ...
    is_meter = fields.Field(
        column_name="Is Meter",
        attribute="asset_json.is_meter",
        widget=wg.BooleanWidget(),
        default=False,
    )
    Code = fields.Field(attribute="assetcode", column_name="Code")
    ...
    GPS = fields.Field(attribute="gpslocation", column_name="GPS Location")
    is_nonengg_asset = fields.Field(
        column_name="Is Non Engg. Asset",
        attribute="asset_json.is_nonengg_asset",
        default=False,
        widget=wg.BooleanWidget(),
    )
    supplier = fields.Field(
        column_name="Supplier", attribute="asset_json.supplier", default=""
    )
    ...
    invoice_no = fields.Field(
        column_name="Invoice No", attribute="asset_json.invoice_no", default=""
    )
    invoice_date = fields.Field(
        column_name="Invoice Date", attribute="asset_json.invoice_date", default=""
    )
    service = fields.Field(
        column_name="Service", attribute="asset_json.service", default=""
    )
    sfdate = fields.Field(
        column_name="Service From Date", attribute="asset_json.sfdate", default=""
    )
    stdate = fields.Field(
        column_name="Service To Date", attribute="asset_json.stdate", default=""
    )
    yom = fields.Field(
        column_name="Year of Manufacture", attribute="asset_json.yom", default=""
    )
    msn = fields.Field(
        column_name="Manufactured Serial No", attribute="asset_json.msn", default=""
    )
    bill_val = fields.Field(
        column_name="Bill Value", attribute="asset_json.bill_val", default=""
    )
    bill_date = fields.Field(
        column_name="Bill Date", attribute="asset_json.bill_date", default=""
    )
    purchase_date = fields.Field(
        column_name="Purchase Date", attribute="asset_json.purchase_date", default=""
    )
    inst_date = fields.Field(
        column_name="Installation Date", attribute="asset_json.inst_date", default=""
    )
    po_number = fields.Field(
        column_name="PO Number", attribute="asset_json.po_number", default=""
    )
    far_asset_id = fields.Field(
        column_name="FAR Asset ID", attribute="asset_json.far_asset_id", default=""
    )
