...
def test_jobform_valid_submission(client_bt, bu_bt, rf):
    ...
    shift = Shift.objects.create(
        shiftname="Morning", enable=True, starttime="08:00", endtime="10:00"
    )
...
@pytest.mark.django_db
def test_adhoc_task_form_valid(
    client_bt, bu_bt, people_factory, asset_factory, questionset_factory, rf
):
    request = rf.post("/")
    request.session = {
