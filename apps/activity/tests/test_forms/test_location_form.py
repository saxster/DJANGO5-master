...
def request_with_session(client_bt, bu_bt):
    ...
    test_tenant, _ = Tenant.objects.get_or_create(
        subdomain_prefix="test_loc",
        defaults={"tenantname": "Test Location Tenant"}
    )
    ...
    request.session["tenant_id"] = test_tenant.id
    request.session.save()
    # Store references for easy access in tests
    request.client_bt = client_bt
    request.bu_bt = bu_bt
    request.test_tenant = test_tenant
    return request


@pytest.mark.django_db
def test_locationform_valid_submission(request_with_session):
    loc_type = TypeAssist.objects.create(
        tacode="LOCATIONTYPE",
        taname="Location Type",
        client=request_with_session.client_bt,
        tenant=request_with_session.test_tenant
    )
    form_data = {
        "loccode": "BLR01",
...
def test_locationform_invalid_gpslocation(request_with_session):
    form = LocationForm(
        data={"loccode": "XYZ", "locname": "GPS Invalid"}, request=request_with_session
    )
...
def test_locationform_valid_gpslocation_parsing(request_with_session):
    form = LocationForm(
        data={"loccode": "XYZ", "locname": "GPS Test"}, request=request_with_session
    )
