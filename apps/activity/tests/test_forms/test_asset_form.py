...
def test_service_from_after_to_raises_error(rf, client_bt):
    ...
    form = AssetExtrasForm(
        data={"sfdate": "2023-02-01", "stdate": "2023-01-01"}, request=request
    )

    assert not form.is_valid()
    assert "Service from date should be smaller than service to date!" in str(
        form.errors
    )
...
def test_invalid_gps_raises_error(rf):
    ...
    form = SmartPlaceForm(
        data={"assetcode": "SP02", "assetname": "SmartNode"}, request=request
    )
