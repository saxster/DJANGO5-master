...
class TestAssetViews:
    ...
    def test_get_assetdetails_returns_valid_response(
        self, authenticated_request, asset_factory
    ):
    ...
    def test_get_assetchart_data_structure(self, authenticated_request, asset_factory):
        ...
        asset_factory(
            assetcode="CHART002", runningstatus="MAINTENANCE", client=bt, bu=bt
        )
        ...
        maintenance_assets = Asset.objects.filter(
            runningstatus="MAINTENANCE", client=bt
        )
    ...
    def test_asset_creation_with_gps_location(
        self, authenticated_request, client_bt, bu_bt
    ):
        """Test asset creation with GPS coordinates"""
        from django.contrib.gis.geos import Point

        # Create service provider as it's required
        servprov = Bt.objects.create(
            bucode="SERV01", buname="Service Provider", enable=True
        )
    ...
    def test_asset_running_status_validation(
        self, client_bt, bu_bt, running_status, expected_valid
    ):
        ...
        # Create service provider as it's required
        servprov = Bt.objects.create(
            bucode="SERVTEST", buname="Service Provider Test", enable=True
        )
