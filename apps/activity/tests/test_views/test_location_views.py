...
class TestLocationViews:
    ...
    def test_get_locationlistview_returns_valid_response(
        self, authenticated_location_request, location_factory
    ):
    ...
    def test_location_str_representation(self, location_factory):
        ...
        location = location_factory(loccode="STR001", locname="String Test Location")
        assert str(location) == "String Test Location (STR001)"

    def test_location_manager_methods(
        self, authenticated_location_request, location_factory
    ):
    ...
    def test_location_search_filtering(
        self, authenticated_location_request, location_factory
    ):
        ...
        location1 = location_factory(
            loccode="SEARCH001", locname="Main Building", client=bt, bu=bt
        )
        location2 = location_factory(
            loccode="SEARCH002", locname="Secondary Building", client=bt, bu=bt
        )
        location3 = location_factory(
            loccode="SEARCH003", locname="Parking Area", client=bt, bu=bt
        )

        # Test search by name
        building_locations = Location.objects.filter(
            locname__icontains="Building", client=bt
        )
        assert building_locations.count() == 2

        # Test search by code
        main_location = Location.objects.filter(loccode="SEARCH001", client=bt)
        assert main_location.exists()

    def test_location_choices_for_report(
        self, authenticated_location_request, location_factory
    ):
        ...
        enabled_location = location_factory(
            loccode="RPT001", locname="Enabled Location", enable=True, client=bt, bu=bt
        )
    ...
    def test_location_bulk_operations(self, location_factory, client_bt, bu_bt):
        ...
        # Test bulk disable
        Location.objects.filter(loccode__startswith="BULK", client=client_bt).update(
            enable=False
        )

        # Verify all are disabled
        disabled_count = Location.objects.filter(
            loccode__startswith="BULK", enable=False, client=client_bt
        ).count()
