...
def test_questions_of_client(session_request):
    q = Question.objects.create(
        quesname="Test Question", client_id=session_request.session["client_id"]
    )
...
def test_get_questions_of_qset(session_request):
    ...
    question = Question.objects.create(
        quesname="Q1", client_id=session_request.session["client_id"]
    )
    qsb = QuestionSetBelonging.objects.create(
        ...
        client_id=session_request.session["client_id"],
        bu_id=session_request.session["bu_id"],
        cuser=session_request.user,
        muser=session_request.user,
        ctzoffset=0,
        seqno=1,
        cdtz=datetime.now(),
        mdtz=datetime.now(),
