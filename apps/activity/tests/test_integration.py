...
def authenticated_client(people_factory):
    ...
    bt = Bt.objects.create(
        bucode="INTTEST", buname="Integration Test Client", enable=True
    )
...
class TestActivityIntegration:
    ...
    def test_asset_location_job_integration(
        self, asset_factory, location_factory, job_factory, client_bt, bu_bt
    ):
    ...
    def test_question_job_workflow(
        self, question_factory, questionset_factory, job_factory, client_bt, bu_bt
    ):
        """Test Question and QuestionSet integration with Jobs"""
        # Create question
        question = question_factory(
            quesname="Integration Test Question", answertype="YESNO"
        )

        # Create question set
        qset = questionset_factory(
            qsetname="Integration Test Question Set", client=client_bt, bu=bu_bt
        )
    ...
    def test_bulk_asset_operations(self, asset_factory, client_bt, bu_bt):
        ...
        # Test bulk update
        Asset.objects.filter(assetcode__startswith="BULK", client=client_bt).update(
            enable=False
        )

        # Verify all bulk assets are disabled
        disabled_assets = Asset.objects.filter(
            assetcode__startswith="BULK", enable=False, client=client_bt
        )
        assert disabled_assets.count() == 10

    def test_job_scheduling_workflow(
        self, job_factory, people_factory, client_bt, bu_bt
    ):
        ...
        jobs_by_priority = Job.objects.filter(
            client=client_bt, identifier="TASK"
        ).order_by(
            "-priority"
        )  # Assuming HIGH > LOW alphabetically
    ...
    def test_asset_criticality_workflow(self, asset_factory, client_bt, bu_bt):
        ...
        pump_assets = Asset.objects.filter(
            assetname__icontains="Pump", client=client_bt
        )
    ...
    def test_job_date_range_filtering(self, job_factory, client_bt, bu_bt):
        ...
        today_jobs = Job.objects.filter(fromdate__date=today, client=client_bt)

        current_jobs = Job.objects.filter(
            fromdate__lte=now, uptodate__gte=now, client=client_bt
        )

        future_jobs = Job.objects.filter(fromdate__date__gt=today, client=client_bt)
    ...
    def test_location_gps_integration(self, location_factory, client_bt, bu_bt):
        ...
        for code, name, gps in locations_data:
            location = location_factory(
                loccode=code, locname=name, gpslocation=gps, client=client_bt, bu=bu_bt
            )
    ...
    def test_audit_trail_integration(
        self, asset_factory, people_factory, client_bt, bu_bt
    ):
    ...
    def test_cascade_deletion_behavior(
        self, asset_factory, job_factory, client_bt, bu_bt
    ):
...
class TestActivityPerformance:
    ...
    def test_bulk_asset_creation_performance(self, client_bt, bu_bt):
        ...
        assert (
            creation_time < 5.0
        ), f"Bulk creation took {creation_time:.2f}s, should be < 5s"
    ...
    def test_complex_query_performance(
        ...
        complex_query = (
            Asset.objects.filter(client=client_bt, iscritical=True, enable=True)
