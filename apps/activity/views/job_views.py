...
class PPMView(LoginRequiredMixin, View):
    ...
    def get(self, request, *args, **kwargs):
        ...
        if R.get("template"):
            return render(request, P["template_list"], context=cxt)
        ...
        if R.get("action", None) == "form":
            cxt = {"ppmform": P["form"](request=request), "msg": "create PPM requested"}
        ...
        # return form with instance
            ...
            cxt = {
                "ppmform": P["form"](instance=ppm, request=request),
...
class PPMJobneedView(LoginRequiredMixin, View):
    ...
    def get(self, request, *args, **kwargs):
        ...
        if R.get("action") == "jobneed_ppmlist":
            objs = P["model"].objects.get_ppm_listview(
...
class CalendarView(View):
    def get(self, request):
        ...
        # Check if modern view is requested
        if R.get("modern") == "true":
            return render(request, "activity/calendar_modern.html")
        ...
        # Default to modern calendar, fallback to old if specified
        if R.get("old") == "true":
            return render(request, "activity/testCalendar.html")
        else:
            return render(request, "activity/calendar_modern.html")
...
class AdhocTasks(LoginRequiredMixin, View):
    ...
    def get(self, request, *args, **kwargs):
        ...
        if R.get("action", None) == "list" or R.get("search_term"):
            total, filtered, objs = self.params[
                "model"
            ].objects.get_adhoctasks_listview(R, self.params["idf"])
        ...
        if R.get("action", None) == "form":
            cxt = {
                "adhoctaskform": self.params["form_class"](request=request),
...
class AdhocTours(LoginRequiredMixin, View):
    ...
    def get(self, request, *args, **kwargs):
        ...
        # then load the table with objects for table_view
        if R.get("action", None) == "list" or R.get("search_term"):
            total, filtered, objs = self.params["model"].objects.get_adhoctour_listview(
                R
            )
            return rp.JsonResponse(
                data={
