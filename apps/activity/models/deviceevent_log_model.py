...
class DeviceEventlog(BaseModel, models.Model):
    ...
    eventvalue = models.CharField(
        _("Device Event"), max_length=50, choices=DeviceEvent.choices
    )
    locationserviceenabled = models.BooleanField(
        _("Location Serivice Enabled"), default=False
    )
    islocationmocked = models.BooleanField(_("Location Spoofed"), default=False)
    locationpermission = models.CharField(
        max_length=25,
        choices=LocationAllowedChoices.choices,
        default=LocationAllowedChoices.NONE.value,
    )
    ...
    receivedon = models.DateTimeField(
        _("Received On"), auto_now=False, auto_now_add=True
    )
    ...
    availintmemory = models.CharField(
        _("Available Internal Memory"), max_length=50, default="NA"
    )
    availextmemory = models.CharField(
        _("Available External Memory"), max_length=50, default="NA"
    )
    signalbandwidth = models.CharField(
        _("Signal Bandwidth"), max_length=50, default="NA"
    )
    platformversion = models.CharField(
        _("Android Version"), max_length=50, default="NA"
    )
    applicationversion = models.CharField(_("App Version"), max_length=50, default="NA")
    networkprovidername = models.CharField(
        max_length=55,
        choices=NetworkProviderChoices.choices,
        default=NetworkProviderChoices.NONE.value,
    )
