...
class StandaloneDatabaseTester:
    ...
    def test_postgresql_function_performance(
        self, function_name, query, params, target_ms, iterations=50
    ):
        ...
        for i in range(iterations):
            try:
                ...
                if i % 10 == 0 and i > 0:
                    print(
                        f"  Progress: {i}/{iterations} - Current: {execution_time:.2f}ms"
                    )
    ...
    def generate_summary_report(self):
        ...
        if "concurrent_load_test" in self.results:
            ...
            load_passed = (
                load_result["queries_per_second"] > 20
                and load_result["avg_response_time"] < 100  # At least 20 QPS
                and load_result["error_rate"]  # Under 100ms average
                < 10  # Under 10% error rate
            )
