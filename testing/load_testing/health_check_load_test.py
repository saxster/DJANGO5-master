...
class HealthCheckLoadTester:
    ...
    def test_single_endpoint(
        self, endpoint, duration_seconds=60, requests_per_second=10
    ):
        ...
        def make_request(thread_id):
            ...
            while not stop_event.is_set():
                try:
                    ...
                    local_results["status_codes"][status_code] = (
                        local_results["status_codes"].get(status_code, 0) + 1
                    )
                    ...
                    if 200 <= status_code < 400:
                        ...
                        local_results["errors"].append(
                            f"HTTP {status_code}: {response.text[:100]}"
                        )
        ...
        while not result_queue.empty():
            ...
            for code, count in thread_result["status_codes"].items():
                results["status_codes"][code] = (
                    results["status_codes"].get(code, 0) + count
                )
        ...
        if results["response_times"]:
            results["avg_response_time"] = round(
                statistics.mean(results["response_times"]), 2
            )
            results["median_response_time"] = round(
                statistics.median(results["response_times"]), 2
            )
            results["p95_response_time"] = round(
                sorted(results["response_times"])[
                    int(len(results["response_times"]) * 0.95)
                ],
                2,
            )
            results["p99_response_time"] = round(
                sorted(results["response_times"])[
                    int(len(results["response_times"]) * 0.99)
                ],
                2,
            )
        ...
        target_avg_response = (
            100 if endpoint in ["/health/", "/ready/", "/alive/"] else 200
        )
        results["passes_performance"] = (
            results["success_rate"] >= 99.0
            and results.get("avg_response_time", float("inf")) <= target_avg_response
        )
        ...
        print(
            f"      Status: {'✅ PASS' if results['passes_performance'] else '❌ FAIL'}"
        )
    ...
    def test_concurrent_health_checks(self, duration_seconds=120):
        ...
        def test_endpoint_continuously(endpoint):
            ...
            return self.test_single_endpoint(
                endpoint, duration_seconds, requests_per_second=20
            )
    ...
    def test_health_checks_during_app_load(self):
        ...
        for endpoint in ["/health/", "/ready/", "/alive/"]:
            ...
            result = self.test_single_endpoint(
                endpoint, duration_seconds=60, requests_per_second=30
            )
        ...
        print(
            f"   Application load simulation: {app_load_results['requests']} requests, {app_load_results['errors']} errors"
        )
        ...
        for endpoint in ["/health/", "/ready/", "/alive/", "/health/detailed/"]:
            try:
                ...
                try:
                    ...
                    baseline_results[endpoint]["status_field"] = json_response.get(
                        "status"
                    )
                ...
                print(
                    f"      {endpoint}: {response.status_code} ({baseline_results[endpoint]['response_time_ms']:.1f}ms)"
                )
    ...
    def generate_summary_report(self):
        ...
        for endpoint, result in self.results.items():
            if isinstance(result, dict) and "passes_performance" in result:
                ...
                print(
                    f"  {endpoint}: {avg_time}ms avg, {success_rate}% success {status}"
                )
        ...
        for endpoint, result in self.results.items():
            if isinstance(result, dict) and result.get("success_rate", 100) < 99:
                error_endpoints.append(
                    f"{endpoint} ({result['success_rate']}% success)"
                )
...
def main():
    ...
    parser.add_argument(
        "--quick", action="store_true", help="Run quick tests with shorter duration"
    )
