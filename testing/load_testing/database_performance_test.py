...
class DatabasePerformanceTester:
    ...
    def test_postgresql_function_performance(
        self, function_name, query, params, target_ms, iterations=100
    ):
        ...
        for i in range(iterations):
            try:
                ...
                if i % 20 == 0:
                    print(
                        f"  Progress: {i+1}/{iterations} - Current: {execution_time:.2f}ms"
                    )
    ...
    def test_concurrent_database_load(self, num_threads=50, duration_seconds=300):
        ...
        results["total_queries"] = (
            results["successful_queries"] + results["failed_queries"]
        )
        if query_times:
            results["avg_response_time"] = round(statistics.mean(query_times), 2)
            results["p95_response_time"] = round(
                sorted(query_times)[int(len(query_times) * 0.95)], 2
            )
        results["queries_per_second"] = round(
            results["successful_queries"] / duration_seconds, 2
        )
        results["error_rate"] = round(
            (results["failed_queries"] / max(results["total_queries"], 1)) * 100, 2
        )
    ...
    def generate_summary_report(self):
        ...
        for func_name, result in self.results.items():
            if isinstance(result, dict) and "passes_target" in result:
                ...
                print(
                    f"  {func_name}: {result['average_ms']}ms (target: {result['target_ms']}ms) {status}"
                )
        ...
        if "concurrent_load_test" in self.results:
            ...
            load_passed = (
                load_result["queries_per_second"] > 50
                and load_result["avg_response_time"] < 100  # At least 50 QPS
                and load_result["error_rate"]  # Under 100ms average
                < 5  # Under 5% error rate
            )
