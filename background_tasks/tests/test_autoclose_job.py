...
class AutocloseJobTest(TestCase):
    ...
    def test_autoclose_job_no_expired_jobs(
        self, mock_atomic, mock_get_db, mock_get_model
    ):
    ...
    def test_autoclose_job_with_autoclose_notify(
        ...
        self.assertEqual(
            mock_email_instance.to, ["admin@example.com", "manager@example.com"]
        )
    ...
    @patch("background_tasks.tasks.butils.get_email_recipients")
    @patch("background_tasks.tasks.transaction.atomic")
    def test_autoclose_job_with_specific_id(
        self, mock_atomic, mock_get_recipients, mock_get_db, mock_get_model
    ):
